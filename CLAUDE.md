...
| `/summary "<prompt>" [N] [story\|series]` | **Standalone top-level tool — NOT part of the and-shoot authoring chain.** Produces the best story (or series) summary from a one-line prompt. Internally: generate a summary, then loop `[independent blind self-escalating critic → fresh reviser]` up to `N` times (default 3); returns the best summary (assumed = last) + the full per-iteration critique/revision history. Wraps the iterate-to-best workflow `pitch-lab/iterate-to-best/iterate-to-best.workflow.js`; the critic grades on the 10-criterion brutal rubric (`pitch-lab/reviewer-spec.md`) and the generator follows GEN-v2 rules (`pitch-lab/generator-spec.md`). Output history persists to `pitch-lab/iterate-to-best/demo-run.md`. Lives in `pitch-lab/`; see `pitch-lab/RUNBOOK.md` + `pitch-lab/iterate-to-best/README.md`. |
