...
| arbiter | **Meta-layer (improvement).** Neutral decider. (1) Judge mode — scores N variant artifacts against a rubric, ranks, names a winner with per-criterion attribution + scorecard; the dedicated tournament/`/and-forge` judge (replaces `general-purpose`). (2) Arbiter mode — rules contested decisions against a fixed standard; enforces the DEC-0115 design-inherent circuit breaker; ESCALATEs persona/irreversible/DEC-overturning calls. Decides between things on the table (admin *proposes*, audience *reacts*, orchestrator-critic *is the standard*). See Rule 23. | `staff/arbiter/` |
| ingrid | **Meta-layer (improvement).** Retrospective + improvement coordinator. Surveys the four signal sources, ranks improvement asks at book/series scope, dispatches the fixes. Principal-delegated authority to edit non-persona agent `.md` + specs/schemas/commands + greenlight non-persona card mutations + dispatch. Persona content → principal (non-delegable). Standing owner of book/series-close retrospective. See Rule 23. | `staff/ingrid/` |
| oskar | **Meta-layer (improvement).** Foreman / routing-optimizer. Triages findings by impact-to-cost, tracks cross-chapter patterns, routes corrections, writes save-as-new NON-PERSONA card mutations (margit reconciles), owns meta-layer housing (memory schemas, STM formats, tooling). The optimizer to the arbiter's grader. Distinct from `studio` (set-manager). See Rule 23. | `staff/oskar/` |
| artur | **Meta-layer (improvement).** Janitor / hygiene-keeper. Repo-wide hygiene over `cards/`, agent memory, indexes, parking-lot, state files. Trivial-fix allowlist only; everything substantive routes (margit / oskar / admin→principal). See Rule 23. | `staff/artur/` |
...
staff/            — production staff: agent homes + audience persona library
  ...
  admin/          — agent home: ltm/stm/goals/methodology/decisions/process-proposals
  arbiter/        — meta-layer agent home: ltm/stm/rulings (judge + arbiter)
  ingrid/         — meta-layer agent home: ltm/stm + retro memos + tend-state (the /and-tend cadence ledger)
  oskar/          — meta-layer agent home: ltm/stm/patterns (cross-chapter pattern ledger)
  artur/          — meta-layer agent home: ltm/stm + reports/ (hygiene sweeps)
  forge/          — /and-forge working dirs + run reports (improvement harness)
...
23. **The improvement layer is one routine conducting a meta-pyramid (brighid consolidation, 2026-06-13).** The work of improving the library, the agents, and the processes is consolidated into a single recurring routine — **`/and-tend`** — designed to be fired by a scheduled trigger several times a day. It is work-queue + cadence driven (`staff/ingrid/tend-state.md` decouples cadence from frequency, so extra runs no-op), defers while a chapter cascade is in flight, is budget-bounded, and mirrors the RUNBOOK R1/R3/R4/R5 discipline (no `AskUserQuestion` — admin user-proxy; ESCALATE queued to the digest; pre-flight-then-silent; single end-of-run digest; clean halts). It has full authority within owning-agent rules: it may dispatch the seats to make real fixes, but **persona content always escalates to the principal** (never auto-applied) and **non-persona card mutations are always save-as-new** (margit reconciles). The meta-pyramid it conducts was ported from brighid-creative-writing (`design/brighid-consolidation.md` is the manifest):
    - **ingrid** — retrospective + improvement coordinator (surveys four signal sources, ranks at book/series scope, dispatches fixes; principal-delegated authority over non-persona agent `.md`/specs/schemas/commands + non-persona card-mutation greenlight; persona content non-delegable).
    - **oskar** — foreman / routing-optimizer (impact-to-cost triage, cross-chapter `patterns.md`, save-as-new non-persona card mutations, owns meta-layer housing). Distinct from `studio` (set-manager).
    - **artur** — janitor / hygiene-keeper (taxonomy/index/memory/parking-lot/state sweeps; trivial-fix allowlist only; everything else routes).
    - **arbiter** — neutral decider (judge mode = the dedicated tournament/`/and-forge` scorer, replacing `general-purpose`; arbiter mode = rules disputes + enforces the DEC-0115 circuit breaker). The grader to oskar's optimizer.
    
    **`/and-forge`** is the agent-parameterized training harness (the editor/revisor improvement vehicle): variants (oskar) → battery → **arbiter** judges → promote winner (non-persona) or escalate (persona). The separation of powers is load-bearing: **admin proposes** process change, **arbiter decides** contests, **audience reacts** as readers, **orchestrator-critic is the standard**, **oskar routes**, **ingrid coordinates**, **artur cleans**, **margit keeps**. Deferred (flagged in the manifest): rewiring `/and-stitch` Phase 1.5/3 tournament to dispatch arbiter instead of `general-purpose` — a live-gate behavior change, not an import.
...
| `/and-forge <agent-slug> [--target <path>] [--cases <path>] [--variants N]` | **Improvement (not the chain).** Training harness for a tunable agent. Generates config variants (oskar) → runs them on an evidence battery → **arbiter** judges blind → promotes the winner (non-persona; persona content escalates). Pre-wired targets `editor` + `fixer` (revisor). Subsumes brighid's editor-/critic-persona-tuning. Always fires admin process-critic. See Rule 23. |
| `/and-tend [--budget N] [--force] [--only <module>] [--dry-run]` | **Improvement (not the chain).** The single consolidated improvement routine — runs several times a day, work-queue + cadence driven, defers on live cascade, no-op-friendly. Conducts the meta-layer pyramid (artur / oskar / ingrid / margit / arbiter / admin / `/and-forge`) to improve the library, agents, and processes. Full authority within owning-agent rules (persona content escalates; non-persona mutations save-as-new). Mirrors RUNBOOK R1/R3/R4/R5. See Rule 23. |
