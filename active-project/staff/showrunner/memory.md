  ...
  - slug: b01
    ...
    chapters:
      - slug: b01c01
        ...
        status: audited-r1    # 2026-05-24 Phase 5b cycle-3 COMPLETE. Final disposition: 8 ACCEPT 3-of-3 (cleared cycle-2 or cycle-3) + 3 deferred-with-attribution (pulp-enthusiast arbiter ruling A) + 1 cap-burn DELETE (dialogue-coll-net-mender-flea-bottom:1 @8). Cycle-3 fixer landed A1/A2 narrator content recasts + B1-B5 vibes token + license trim (cleared cycle-2 NI + vibes dissents); C1 state-updates-Coll ADD pre-validation FAILED on three rubric axes (URI-FACETS-CYCLE-N-ADD A3) → cap-burn DELETE per URI-FACETS-CAP-BURN-SEMANTICS A2. Status flip per A2 step 6: cap-burn DELETE treated as resolution path; audience-gate resolved-even-if-NOT-SUCCESSFULly. orchestrator_critic_verdict NOT-SUCCESSFUL recorded but NOT a HARD-BLOCK against /and-stitch.
        audit_path: active-project/staff/auditor/facets-final-audit-cycle3.md    # 2026-05-24 post-cycle-3 audit; 3 HARD all ACCEPTED (fault-030 bare-speech-bone @8 + fault-031 empty Coll speaker file body — both ACCEPTED-AT-CAP-BURN; fault-021 sensory exemption carry); 7 SIGNAL (cycle-2 carry-through; fault-017 + fault-020 MOOT post-B1 license-trim + post-§Form directive); 0 unexpected HARDs from A1/A2/B1-B5 edits; AP8 PASS on both borderline new vibes tokens; bidirectional-loop convergence VALIDATED preserved
        audit_complete: true    # cycle-3 mechanical audit complete; HARDs all ACCEPTED via cap-burn or carry-as-adjudicated
        audit_findings: 10    # 3 HARD ACCEPTED + 7 SIGNAL; cap-burn disposition documented in cap-burn report
        prior_audit_path_cycle2: active-project/staff/auditor/facets-final-audit-cycle2.md    # historical cycle-2 audit (HARD=0, 9 SIGNAL); superseded by cycle-3 audit
        audience_gate_path: active-project/staff/auditor/facets-audience-gate-r3.md    # 2026-05-24 cycle 3 of /and-facets b01-c01 Phase 5b (final cycle); supersedes cycle-2 r2 report (kept as prior_audience_gate_path_cycle2 for historical reference)
        prior_audience_gate_path_cycle2: active-project/staff/auditor/facets-audience-gate-r2.md    # historical cycle-2 report (5 ACCEPT / 6 FAIL pre-arbitration → 9 ACCEPT / 3 FAIL post-arbitration); see § "Orchestrator-judgment arbitration" for the three pulp-enthusiast ruling-A cases
        audience_gate_cap_burn_path: active-project/staff/auditor/facets-cap-burn-b01c01-20260524T021822Z.md    # 2026-05-24 cap-burn report; documents the C1 dialogue-coll DELETE + the two ACCEPTED-AT-CAP-BURN HARDs + the rubric-edit Path (b) follow-on flag + the cite-index builder dialogue-dir scan patch (commit b6df1e6)
        audience_gate_complete: true    # cycle 3 final: 8 ACCEPT 3-of-3 + 3 deferred-with-attribution + 1 cap-burn DELETE / 0 fail; resolution-path gate met (all 12 facets carry a documented disposition); strict-aggregation gate not met (3 deferred + 1 cap-burned)
        audience_gate_cycle: 3    # of cap 3 (URI-FACETS-CAP-BURN-SEMANTICS A2); cap exhausted via DELETE-on-cycle-3 path
        audience_gate_cap_burned: true    # cap-burn DELETE executed on dialogue-coll-net-mender-flea-bottom:1 @8 per URI-FACETS-CAP-BURN-SEMANTICS A2
        cap_burn_deletions: ["coll-net-mender-flea-bottom:1 @8"]    # the single facet entry deleted at cycle-3 cap-burn; cite-index cascade auto-stripped [coll-net-mender-flea-bottom:1] from proto-line @8 via build_cite_index.py extended-to-dialogue-dir patch (commit b6df1e6)
        audience_gate_facets_passed: [dialogue-wren-stitch-maker-flea-bottom-ward, location-state, state-updates, feeling, metaphor, exposition, interest-narrator, vibes]    # 8 facets cleared 3-of-3 (1 cycle-1 + 5 cycle-2 + 2 cycle-3: interest-narrator post-A1/A2 + vibes post-B1-B5)
        audience_gate_facets_deferred_with_attribution: [memory, sensory, dialogue-taylor-hebert-kl-122ac]    # 3 facets cleared via pulp-enthusiast taste-arbiter ruling A (defer-as-taste); dissents recorded as attribution trail; arbiter verdict at staff/audience/pulp-enthusiast/orchestrator-judgment-cycle2-arbiter-verdict.md
        audience_gate_facets_failed: []    # post-cycle-3: cleared (interest-narrator + vibes cleared 3-of-3; dialogue-coll moved to cap_burn_deletions)
        audience_gate_arbiter_path: active-project/staff/audience/pulp-enthusiast/orchestrator-judgment-cycle2-arbiter-verdict.md    # 2026-05-23 pulp-enthusiast taste-arbiter fork; ruled A (defer-as-taste) on all three orchestrator-judgment cases (memory mem:1, dialogue-taylor taylor:2, sensory shape-level)
        ...
        audience_gate_prior_run_stale: false    # 2026-05-24 — the pre-redo cap-burned chain was replaced by the cycle-1/2/3 redo chain; cycle-3 of the redo chain completed this date; no stale-prior-run conflict outstanding. Historical pointer (pre-redo): prior cap-burned r3 on the replaced bone set was logical-only — no disk artifact survives the redo bones reset.
        bidirectional_loop: validated    # cycle-3 preserved; loop structure unchanged across redo + cap-burn (auditor + audience-gate cross-check held; cap-burn HARDs are convergent across both paths via cycle-2 dark-fantasy DEMAND on @8 + cycle-3 auditor structural finding)
        facets_path: active-project/theater/facets/
        round_1_complete: true    # post-cycle-3 — R1 fanout completed against current bone set; facet authoring locked
        round_2_complete: true    # post-cycle-3 — R2 fanout judgments locked; cycle-3 fixer edits applied directly to consolidated files (per --only-prefix discipline)
        orchestrator_critic_verdict: NOT-SUCCESSFUL    # 2026-05-24 cap-burn verdict per URI-FACETS-CAP-BURN-SEMANTICS A2 step 5; recorded but NOT a HARD-BLOCK against /and-stitch; cap-burn DELETE of dialogue-coll:1 @8 + 2 ACCEPTED-AT-CAP-BURN HARDs (fault-030 bare-speech-bone + fault-031 empty Coll speaker file body) drive the verdict; cap-burn report at staff/auditor/facets-cap-burn-b01c01-20260524T021822Z.md
