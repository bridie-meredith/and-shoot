...
state_axes:
  ...
  - slug: position
    ...
    start_rank_justification: '122 AC world-notes: Otto off council but operating advisory; Alicent faction consolidated but succession still contested'
  ...
  # loading onto the un-priced anchor (Wren / cost-bearer). Rank 1 = anchor
  # exists, no weight on the unpricing. Rank 9 = unpricing is structurally
  # causal to the catastrophe. Taylor's ledger exclusion is held constant;
  # what changes is what that exclusion costs.

  - slug: relational_anchor_status
    dimension: 'operational weight loading onto the un-priced relational anchor (Wren)'
    one_means: 'anchor present; no operational weight — Taylor is attached, Wren not yet in any calculus'
    five_means: 'anchor inside the protection architecture without Wren's knowledge or consent; Taylor routing around use-vectors'
    nine_means: 'Wren's exclusion from the ledger is structurally causal to her death — the un-priced item is the one Taylor could not defend'
    perspective: protagonist
    start_rank: 1
    end_rank: 9
    class: emotional
    notes: 'monotonic rise in pressure; the relationship itself does not change — the weight of not-pricing it does; Wren never enters the ledger; d14 is the revelation'
  ...
  # Otto never identifies Wren as leverage; the pressure is structural not agential.
...
cost_ledger:
  ...
  - id: cl01
    ...
    cost: 'relational_anchor_status +2'
    anchor: { book: b01, chapter: null, scene: null }
    notes: 'd01 rescue: capability deployed (first use since Gold Morning); social_tether opens into Flea Bottom; witch-label witnesses load the un-priced anchor with its first operational weight'
  ...
  - id: cl05
    gain: 'capability +2, moral_framework-collapse-confirmed'
    cost: 'moral_framework -2'
    anchor: { book: b01, chapter: null, scene: null }
    notes: 'd12 full deployment: network pushed to full ward coverage to protect Sera; scale smaller than Gold Morning, moral shape the same; irrevocable-Khepri-repetition confirmed'
  ...
  - id: cl07
    gain: 'nothing'
    cost: 'relational_anchor_status +4, social_tether -7, position -6, moral_legibility_to_self +4'
    anchor: { book: b01, chapter: null, scene: null }
    notes: 'd14 burn: the final ledger entry cashes everything; Wren dead in the mapped streets; Taylor removed; moral_legibility reaches recognition-too-late; the accounting delivers its final output and she receives it'
...
antagonist_pressure:
  ...
  - axis: relational_anchor_status
    ...
    cost_curve: 'escalates monotonically from d01 (witnesses at rescue) through d04 (network threads through wards); caps when Taylor routes around vectors that might expose Wren to Otto'
...
chunk_targets:
  ...
  book:    { delta_per_signature_axis: 3-4, density_target: 0.7-0.9, bone_count: 270-330 }
...
position                     | world        |   4   |  9  |  +5   | monotonic rise
...
political_register_toward_elite | world      |  4   |  9  |  +5   | monotonic rise (court consolidates)
```
...
─────|────────────────────────────────────────|────────────────────────────────────────|──────────────────
cl01 | capability +2, social_tether +2        | relational_anchor_status +2            | d01
...
cl07 | nothing                                | relational_anchor +4, tether -7,       | d14
     |                                        | position -6, legibility +4             |
```

**Authoring note — cl01 cost field:** `relational_anchor_status +2` is an increase in pressure, not a gain. The cost-ledger `cost:` field here records the axis movement that makes the rescue a trade: deploying capability in public loads the un-priced anchor with its first weight. If the schema requires the cost field to read as a decrement, substitute `opportunity-missed: rescue is witnessed; witch-label forms; Wren's block enters the exposure radius`.

**Authoring note — cl04 and cl06:** both use `opportunity-missed` form because the cost is exit-foreclosure (no axis to decrement; the trade forecloses a path, not an asset).

**Authoring note — cl07:** the gain field is `nothing` by design. d14 is pure cost-cashing. No protagonist axis ends higher than it was before d14 fires. The only thing that rises is moral_legibility_to_self (recognition-too-late = highest clarity Taylor ever reaches) — and that rise is also the cost.
