# /and-substance chapter b01c04 — scene chunks draft (SUPERSEDED — persisted to memory.md 2026-05-27)
# Phase 2 + 3 + 4 output from screen-writer; persisted to memory.md chapters[b01c04].scenes[] at Phase 6.
# Archived per URI-SUBSTANCE-DRAFT-PRUNE 2026-05-27.
# s01 cl-antag-d03 "third tranche" note in this archived draft already reflects fault-002 correction (DEC-0028);
#   the original "cl-antag-d03 completed" claim was caught by auditor at Phase 5 and fixed inline before archive.
    ...
    substance_delta:
      axes_in_motion:
        - axis: social_tether-antag
          ...
          notes: "acceptance delivered and acknowledged; Otto's lever solidified from embryonic to operational; cl-antag-d03 third tranche (+1.0 of +4 total; c03 already delivered +1.5, c04 adds +1.0, +1.5 still outstanding for d05–d10 chapters before cl-antag-d10 begins); Taylor can no longer un-be-the-intelligence-source"
...
# Per-axis sums vs chapter target:
#   capability: s01(0) + s02(1.0) + s03(0.5) = 1.5 vs +1.5 (target) → EXACT
#   position-prot-rise: s01(0.5) + s02(0) + s03(0.5) = 1.0 vs +1.0 (target) → EXACT
#   social_tether-prot-rise: s01(0) + s02(1.0) + s03(1.0) = 2.0 vs +2.0 (target) → EXACT
#   social_tether-antag: s01(1.0) + s02(0) + s03(0) = 1.0 vs +1.0 (target) → EXACT
#   position-world: s01(0) + s02(0) + s03(1.0) = 1.0 vs +1.0 (target) → EXACT
#
# URI-CONTRACT-THEMATIC-AXIS check:
#   chapter goal axis (social_tether-prot-rise) is stakes_axis for s02 AND s03 → SATISFIED
#
# Constraint check (pre-review):
#   - POV: third-limited Taylor throughout; no POV violations
#   - Earth-Bet proper-noun fence: no parahuman jargon; "insects" / "feed" only
#   - No on-stage dragons: absent
#   - Oswyn Mudway: observed, not contacted; unknowing node confirmed
#   - Wren: in expanded coverage, outside report and outside pricing; explicitly named as such in s03
#   - Otto: off-stage; felt through Jarvis's confirmation
#   - Action-only bullet rule (mode: plan generation): N/A — this is YAML chunk format, not bullet list
#   - Motivation clauses: chunks name what happens; causation is implicit or dramatized, not pre-told
