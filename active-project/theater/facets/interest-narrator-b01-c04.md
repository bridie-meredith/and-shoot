...
# ── Rubric carve-out preamble (body-level; audit fault-004 + fault-008 + Phase 5b worm-canon arithmetic correction 2026-05-27) ──
...
entry-count breakdown (post-R2, post-fault-006):
  - state-co-citation-mandatory (per rubric-state-updates §cross-facet contract; one NI per actor:taylor.* entry):
      narrator:3 @9 (paired state:1 social_tether-antag delivery)
      narrator:11 @15 (paired state:3 capability extension day-1)
      narrator:4 @18 (paired state:4 Oswyn unknowing-node)
      narrator:6 @22 (paired state:4/5 Wren-coverage + non-report exclusion)
      narrator:13 @27 (paired state:6/7 capability completion day-2)
      narrator:7 @31 (paired state:7/10 social_tether-prot-rise routing)
    = 6 mandatory fires (worm-canon arithmetic accepted; prior preamble claimed 7 — corrected)

  - non-state-mandatory structural fires:
      narrator:1 @6 (scene-A approach to peak; feed-priority cost-tracking)
...
resolves:
  ...
  - audit fault-008 (RUBRIC-FIDELITY NI band overshoot without machine-parsable carve-out)
  - Phase 5b cycle-1 worm-canon REVISE arithmetic discrepancy (preamble math corrected: 6 mandatory + 5 non-mandatory = 11; prior 7+4 claim was wrong)
