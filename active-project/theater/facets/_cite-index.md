# Cite-Index — b01-c01
generated: 2026-05-25
source: active-project/theater/proto-lines/b01-c01.md + active-project/theater/facets/
scope: 9 facet files + 1 proto-lines file
totals: 44 facet entries; 16/27 protolines decorated (59.3%)

## Density distribution (protolines by citation count)

| cites/line | count |
|------------|-------|
| 0 (bare)   | 11 |
| 1          | 9 |
| 2          | 1 |
| 3          | 2 |
| 4          | 1 |
| 6          | 2 |
| 8          | 1 |

## Per-facet entries

### loc-state (0 entries)
_(no entries)_

### narrator (8 entries)
  narrator:1 @4 back=Y
  narrator:2 @8 back=Y
  narrator:3 @11 back=Y
  narrator:4 @12 back=Y co=[state:3, state:4, vibes:3, vibes:4, vibes:8]
  narrator:5 @21 back=Y co=[exposition:8, feel:1, state:1, state:6, vibes:5, vibes:6, vibes:7]
  narrator:6 @27 back=Y co=[exposition:9, feel:4, state:9, vibes:9, vibes:10]
  narrator:7 @3 back=Y co=[mem:1]
  narrator:8 @24 back=Y co=[state:7]

### sensory (2 entries)
  sensory:1 @2 back=Y
  sensory:2 @16 back=Y co=[taylor-hebert-kl-122ac:1, taylor-hebert-kl-122ac:2, taylor-hebert-kl-122ac:3]

### state (9 entries)
  state:1 @21 back=Y co=[exposition:8, feel:1, narrator:5, state:6, vibes:5, vibes:6, vibes:7]
  state:2 @26 back=Y co=[mem:2, state:8]
  state:3 @12 back=Y co=[narrator:4, state:4, vibes:3, vibes:4, vibes:8]
  state:4 @12 back=Y co=[narrator:4, state:3, vibes:3, vibes:4, vibes:8]
  state:5 @17 back=Y
  state:6 @21 back=Y co=[exposition:8, feel:1, narrator:5, state:1, vibes:5, vibes:6, vibes:7]
  state:7 @24 back=Y co=[feel:3, narrator:8]
  state:8 @26 back=Y co=[mem:2, state:2]
  state:9 @27 back=Y co=[exposition:9, feel:4, narrator:6, vibes:9, vibes:10]

### mem (2 entries)
  mem:1 @3 back=Y co=[narrator:7]
  mem:2 @26 back=Y co=[state:2, state:8]

### feel (4 entries)
  feel:1 @21 back=Y co=[exposition:8, narrator:5, state:1, state:6, vibes:5, vibes:6, vibes:7]
  feel:2 @10 back=Y
  feel:4 @27 back=Y co=[exposition:9, narrator:6, state:9, vibes:9, vibes:10]

### meta (0 entries)
_(no entries)_

### vibes (10 entries)
  vibes:1 @- back=-
  vibes:2 @- back=-
  vibes:3 @12 back=Y co=[narrator:4, state:3, state:4, vibes:4, vibes:8] lic-out=[proto:12, proto:13]
  vibes:4 @12 back=Y co=[narrator:4, state:3, state:4, vibes:3, vibes:8] lic-out=[proto:12, proto:13, proto:14]
  vibes:5 @21 back=Y co=[exposition:8, feel:1, narrator:5, state:1, state:6, vibes:6, vibes:7] lic-out=[proto:21, proto:26]
  vibes:6 @21 back=Y co=[exposition:8, feel:1, narrator:5, state:1, state:6, vibes:5, vibes:7] lic-out=[proto:21, proto:26]
  vibes:7 @21 back=Y co=[exposition:8, feel:1, narrator:5, state:1, state:6, vibes:5, vibes:6] lic-out=[proto:21, proto:26]
  vibes:8 @12 back=Y co=[narrator:4, state:3, state:4, vibes:3, vibes:4] lic-out=[proto:12, proto:13, proto:14, proto:21]
  vibes:9 @27 back=Y co=[exposition:9, feel:4, narrator:6, state:9, vibes:10] lic-out=[proto:27, proto:2, proto:25]
  vibes:10 @27 back=Y co=[exposition:9, feel:4, narrator:6, state:9, vibes:9] lic-out=[proto:27, proto:2, proto:15]

### exposition (9 entries)
  exposition:1 @0 back=N
  exposition:2 @0 back=N
  exposition:3 @0 back=N
  exposition:4 @1 back=N co=[exposition:5]
  exposition:5 @1 back=Y lic-in=[exposition:8, exposition:9]
  exposition:6 @7 back=Y
  exposition:7 @18 back=Y
  exposition:8 @21 back=Y co=[feel:1, narrator:5, state:1, state:6, vibes:5, vibes:6, vibes:7] lic-out=[exposition:5]
  exposition:9 @27 back=Y co=[feel:4, narrator:6, state:9, vibes:9, vibes:10] lic-out=[exposition:5]

## Pile-ups (>4 facets co-located on one protoline)

- **@21** (8): exposition:8, feel:1, narrator:5, state:1, state:6, vibes:5, vibes:6, vibes:7
    `oswyn-mudway-flea-bottom-elder takes the lane-mouth`
- **@12** (6): narrator:4, state:3, state:4, vibes:3, vibes:4, vibes:8
    `the insects propagate`
- **@27** (6): exposition:9, feel:4, narrator:6, state:9, vibes:9, vibes:10
    `wren-stitch-maker-flea-bottom-ward faces taylor-hebert-kl-122ac`

## Lonely entries (no co-location, no inbound license)
_Round-2 deletion candidates — but check the rubric before cutting._

- narrator:1 @4  `the insects swell`
- narrator:2 @8  `the ground transmits the child's breath`
- narrator:3 @11  `the lane-mouth presses the crowd`
- sensory:1 @2  `the tallow smoke crosses the stitch-house lane`
- state:5 @17  `taylor-hebert-kl-122ac lifts the hands`
- feel:2 @10  `taylor-hebert-kl-122ac holds the feet`
- exposition:1 @0  ``
- exposition:2 @0  ``
- exposition:3 @0  ``
- exposition:6 @7  `the fish-cart blocks the lane`
- exposition:7 @18  `the crowd thins`

## Bare protolines (no citations accrued)
_Round-2 add candidates if the rubric licenses a fire here._

@5, @6, @9, @13, @14, @15, @19, @20, @22, @23, @25
