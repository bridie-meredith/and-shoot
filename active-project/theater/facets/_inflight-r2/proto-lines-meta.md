---
facet: metaphor
episode: s01e02
layer: R2 judge pass (2026-05-11)
source: active-project/theater/proto-lines/s01e02.md
note: canonical copy — [meta:<id>] annotations show R2 cascade. KEEP verdicts = no cascade change to existing citations. No adds; no deletes. Body-integrity preserved.
---

# proto-lines — s01e02

episode: s01e02
narrator: taylor-hebert-flea-bottom
goal: The tanner-family's claim on Taylor escalates from informal grief to formalized customary wage-claim across three visits; the broken maester transitions from ambient signal to named log entry; range expands from 300m to 400m with first physiological cost; the mother extinguishes the vigil candle — the village's grief has changed shape but the transactional surface does not close.
cast: taylor-hebert-flea-bottom, oc-tanner-father, oc-tanner-mother, oc-tanner-elder, oc-broken-maester, oc-dock-runner
locations: loc-flea-bottom-base
prior_episode: s01e01
aggregate_range: 159-328 (+ interpolated narrative-scope: 496, 500, 501, 502, 505, 508, 509, 510, 511, 512, 519, 520, 521, 526, 527, 528)

1
2

3 taylor-hebert-flea-bottom faces the junction [loc-state:1] [state:1]
4 oc-tanner-elder faces the road [narrator:1]
5 oc-tanner-father crosses the market-side junction [loc-state:2] [state:1]
6 oc-tanner-mother crosses the market-side junction [state:1]
7 oc-tanner-elder greets oc-tanner-father
8 oc-tanner-elder greets oc-tanner-mother
9 taylor-hebert-flea-bottom approaches the junction [narrator:2]
10 the arrival enters the junction
11 taylor-hebert-flea-bottom pivots toward the arrival
12 oc-tanner-father stills [narrator:3]
13 taylor-hebert-flea-bottom faces oc-tanner-father [feel:1]
14 oc-tanner-father speaks to taylor-hebert-flea-bottom
15 taylor-hebert-flea-bottom speaks to oc-tanner-father
16 oc-tanner-father stills
17 oc-tanner-mother stills
18 oc-tanner-father lifts the trade goods [narrator:4]
19 oc-tanner-father speaks to oc-tanner-elder
20 oc-tanner-elder speaks to oc-tanner-father [feel:1] [narrator:5] [state:1] [vibes:4]
21 oc-tanner-mother faces taylor-hebert-flea-bottom
22 oc-tanner-father steps back [narrator:6] [state:2] [state:3] [vibes:1] [vibes:2] [vibes:3]
23 oc-tanner-mother speaks to taylor-hebert-flea-bottom
24 taylor-hebert-flea-bottom speaks to oc-tanner-mother
25 oc-tanner-mother pivots toward the road south [narrator:7]
26 oc-tanner-father pivots toward the road south
27 oc-tanner-father exits the junction [state:4]
28 oc-tanner-mother exits the junction [state:2]
30 taylor-hebert-flea-bottom opens the log [mem:2] [narrator:8]
31 taylor-hebert-flea-bottom writes the entry [state:3]
32 taylor-hebert-flea-bottom closes the log
34 the flies relay oc-tanner-elder
35 the wasps relay the Watch patrol [narrator:9]
37 the wasps relay oc-dock-runner
41 taylor-hebert-flea-bottom enters loc-flea-bottom-base [loc-state:3] [sensory:1] [state:1] [state:4]
42 the flies relay the carter
44 the flies relay the wind
45 taylor-hebert-flea-bottom opens the log
46 taylor-hebert-flea-bottom writes the entry
49 taylor-hebert-flea-bottom closes the log
50 taylor-hebert-flea-bottom enters the market-side junction [loc-state:4]
51 oc-tanner-elder pauses
52 oc-tanner-elder speaks to the carter
53 the wasps relay the pass [narrator:10]
54 oc-tanner-elder faces taylor-hebert-flea-bottom [feel:2]
55 the carter exits the junction
56 taylor-hebert-flea-bottom opens the log
57 taylor-hebert-flea-bottom writes the entry
58 taylor-hebert-flea-bottom closes the log
60 the flies spread the overnight network
61 the wasps spread the dock-side alleys
62 the beetles spread the south-wall colony
63 the spiders spread the eastern-quarter approach
64 taylor-hebert-flea-bottom rolls the shoulders [mem:3] [narrator:11]
65 the flies relay the junction return
66 the wasps relay the Fish Gate perimeter
67 the beetles relay the south-wall return
68 the spiders relay the window
69 taylor-hebert-flea-bottom exhales [narrator:12]
70 the flies spread the northern block
71 the wasps spread the eastern-quarter adjacent
72 taylor-hebert-flea-bottom exhales
73 taylor-hebert-flea-bottom walks the perimeter [state:2]
74 taylor-hebert-flea-bottom writes the entry
75 taylor-hebert-flea-bottom writes the entry
76 taylor-hebert-flea-bottom exhales
77 taylor-hebert-flea-bottom wakes [narrator:13]
78 taylor-hebert-flea-bottom holds the eyes [narrator:14]
79 taylor-hebert-flea-bottom opens the log
80 taylor-hebert-flea-bottom writes the entry
81 taylor-hebert-flea-bottom closes the log
83 the lords-man enters the alley [loc-state:5] [narrator:15]
84 the lords-man speaks to the tenant family
85 the lords-man's man breaks the door latch [narrator:16] [sensory:2] [state:3]
86 the lords-man's man moves the family possessions
87 the tenant family exits the two-room dwelling [feel:2] [narrator:17]
88 the neighbor presses the doorway [mem:4] [narrator:18]
89 the flies relay the lords-man's man [meta:1] [vibes:12] [vibes:13]
# R2 JUDGE NOTE @89: meta:1 KEEP — anchor rewritten from provisional to feel:7 +tens:1. No cascade change to citation list.
90 the beetles relay the door lintel
91 the flies relay the wall
92 the neighbor withdraws
93 taylor-hebert-flea-bottom opens the log
94 taylor-hebert-flea-bottom writes the entry
95 taylor-hebert-flea-bottom closes the log
97 oc-tanner-father enters the market-side junction [loc-state:6] [state:5]
98 oc-tanner-mother enters the market-side junction [state:3]
99 oc-tanner-father faces taylor-hebert-flea-bottom
100 oc-tanner-father speaks to taylor-hebert-flea-bottom [narrator:19] [state:6]
101 taylor-hebert-flea-bottom faces oc-tanner-father
102 taylor-hebert-flea-bottom speaks to oc-tanner-father
103 oc-tanner-father speaks to taylor-hebert-flea-bottom
104 taylor-hebert-flea-bottom exhales [narrator:20]
105 taylor-hebert-flea-bottom opens the purse
106 taylor-hebert-flea-bottom extends the coins [feel:3] [narrator:21] [vibes:5]
107 oc-tanner-father takes the coins [feel:2] [state:5] [state:7] [vibes:6] [vibes:7]
108 oc-tanner-father speaks to taylor-hebert-flea-bottom [narrator:22]
109 taylor-hebert-flea-bottom faces oc-tanner-father
110 oc-tanner-father exits the junction [state:8]
111 oc-tanner-mother exits the junction [state:4]
113 taylor-hebert-flea-bottom opens the log
114 taylor-hebert-flea-bottom writes the entry [meta:2]
# R2 JUDGE NOTE @114: meta:2 KEEP — anchor rewritten from provisional to mem:5 +tens:1. No cascade change to citation list.
115 taylor-hebert-flea-bottom closes the log
117 the flies spread the autumn-density network [state:6] [vibes:8] [vibes:11]
118 the wasps spread the dock-side relay
119 the spiders spread the eastern-quarter relay
120 taylor-hebert-flea-bottom walks the perimeter
121 taylor-hebert-flea-bottom writes the entry
122 taylor-hebert-flea-bottom writes the entry
123 taylor-hebert-flea-bottom flexes the hand
124 taylor-hebert-flea-bottom wakes
125 taylor-hebert-flea-bottom drops the stylus [narrator:23] [sensory:3] [state:7] [vibes:9] [vibes:10]
126 taylor-hebert-flea-bottom holds the eyes
127 taylor-hebert-flea-bottom lowers the chin [narrator:24]
128 taylor-hebert-flea-bottom opens the log
129 taylor-hebert-flea-bottom writes the entry
130 taylor-hebert-flea-bottom closes the log
132 the visitor enters the side alley [loc-state:7]
133 the visitor enters the stairwell
136 the visitor enters the upper room [loc-state:8]
137 the visitor speaks to the maester
138 the maester speaks to the visitor
139 the beetles relay the register [narrator:25]
140 taylor-hebert-flea-bottom holds the feet [narrator:26]
141 the visitor exits the upper room
142 the visitor exits the stairwell
143 the visitor exits the side-alley door
144 taylor-hebert-flea-bottom opens the log
145 taylor-hebert-flea-bottom writes the entry [state:8]
146 taylor-hebert-flea-bottom closes the log
148 the beetles relay the rhythm
149 the beetles relay the phrase [narrator:27] [state:1]
150 the beetles relay the rhythm
151 taylor-hebert-flea-bottom stills [narrator:28] [state:9] [vibes:14]
152 taylor-hebert-flea-bottom opens the log
153 taylor-hebert-flea-bottom writes the entry
154 taylor-hebert-flea-bottom closes the log
156 oc-broken-maester exits the apothecary [loc-state:9]
157 oc-broken-maester enters the side alley
158 oc-broken-maester enters the upper room [loc-state:10]
159 the beetles relay oc-broken-maester [mem:7] [narrator:29] [vibes:15] [vibes:16] [vibes:17]
160 taylor-hebert-flea-bottom opens the log
161 taylor-hebert-flea-bottom writes the entry
162 taylor-hebert-flea-bottom closes the log
164 oc-tanner-mother enters loc-flea-bottom-base [loc-state:11] [sensory:4] [state:5]
165 oc-tanner-mother enters the base room [loc-state:12] [state:6]
166 oc-tanner-mother faces taylor-hebert-flea-bottom
167 oc-tanner-mother sits [state:7]
168 taylor-hebert-flea-bottom faces oc-tanner-mother
169 oc-tanner-mother speaks to taylor-hebert-flea-bottom [vibes:20]
170 taylor-hebert-flea-bottom speaks to oc-tanner-mother [feel:4]
171 oc-tanner-mother speaks to taylor-hebert-flea-bottom [narrator:30] [state:10] [vibes:21]
172 oc-tanner-mother lowers the gaze
173 oc-tanner-mother stands [feel:1] [narrator:31] [sensory:5] [state:4] [state:8] [state:9] [vibes:18] [vibes:19] [vibes:22] [vibes:23]
174 oc-tanner-mother exits the base room [state:10]
176 taylor-hebert-flea-bottom opens the log
177 taylor-hebert-flea-bottom writes the entry [narrator:32]
178 taylor-hebert-flea-bottom closes the log
