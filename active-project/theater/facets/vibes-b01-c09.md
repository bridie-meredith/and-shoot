...
# rubric-carve-out — chapter-internal location without library card (the-hook-ward)
#
# rubric-vibes.md (design/shoot-v2/rubric-vibes.md) § Required gates, gate 1 (Target validity)
#
# Carve-out scope: vibes:4 entry targeting loc:the-hook-ward.
# Carve-out rule: gate 1 requires slug to exist in card library. the-hook-ward is a
#   chapter-internal staging location present in the bones header, scene-map, and
#   location-state facet graph but without a formal card at cards/locations/ or
#   active-project/warehouse/. The c08 vibes facet established this carve-out pattern
#   (vibes-b01-c08.md, carve-out preamble, vibes:1) and authored the first keyword
#   (coverage-extends-through-absorbance). The present entry is a ++ extension of that
#   existing keyword, not a fresh add. Extension compounds on a carve-out-established
#   keyword: same carve-out logic applies. Scoped to this single entry.
# Coverage justification: the-hook-ward's coverage-extends-through-absorbance keyword
#   (c08) established the location as doubly-surveilled ground; c09's s01 adds a
#   person-specific pattern-layer (Wren's daily circuit) above the prior geometric-layer
#   (Oswyn's watcher-network). The ++ extension is load-bearing for studio's future
#   rendering of the hook-ward as a location that carries named-individual pattern-maps
#   not only geometric coverage.
#
# Per-entry annotations:
#   - vibes:4 (no anchor): chapter-internal-location carve-out (gate 1), same carve-out
...
1 @6 actor:taylor-hebert-kl-122ac ++ atonement-as-repetition: [wren-daily-circuit-in-internal-map-unlogged, vulnerable-window-documented-without-naming, pattern-specificity-without-pricing] | licensed-by: proto:6, proto:8, proto:10, state-update:1
  ...
  # relational_anchor_status_axis 2 -> 2.5). This is the same form-fitting-without-naming
  # the pattern has always taken, but at person-specific granularity (a route with
  # waypoints, a vulnerable window) rather than geometric coverage. New event-frame:
  # prior tokens cover enforcement-incident-routing and watcher-network-absorption;
...
2 @6 actor:taylor-hebert-kl-122ac ++ rising entrapment: [daily-pattern-of-named-individual-mapped-and-withheld, coverage-architecture-now-person-specific, vulnerable-window-inside-the-grid] | licensed-by: proto:6, proto:8, proto:10, state-update:1
  ...
  # including when she is alone in a lane with one entrance, is inside the coverage
...
3 @6 actor:wren-stitch-maker-flea-bottom-ward ++ rising entrapment: [daily-circuit-legible-to-the-unseen-observer, alone-window-documented-in-the-grid, mapped-freedom-not-only-constrained-freedom] | licensed-by: proto:6, proto:7, state-update:1, world-build:cond-road-to-hell-chain-shape
  ...
  # corner, the one-block stretch where she is alone — and Taylor now holds the time-block,
  # route, and vulnerable window in the internal map. Wren does not know the circuit is
  # legible; she is simply walking it. The new tokens add the specific form of this
...
4 loc:the-hook-ward ++ coverage-extends-through-absorbance: [named-individual-daily-pattern-mapped-in-lane-grid, vulnerable-lane-window-inside-coverage, wren-circuit-as-person-specific-layer-above-geometric] | licensed-by: proto:6, proto:10, state-update:1
  ...
  # surveilled ground at the geometric level. c09 adds a person-specific layer above that:
  # the lanes between the stitch-shop and water-sellers now hold a named individual's
  # daily pattern including a vulnerable window inside the coverage architecture. Studio
  # reads the hook-ward as carrying both the geometric layer (Oswyn's former watcher-
...
5 @18 actor:taylor-hebert-kl-122ac ++ political-register-color-present: [particular-gate-particular-body-particular-faction-direction, black-faction-contact-inferred-from-posture-and-location, color-sharpening-to-directional-inference-not-substrate] | licensed-by: proto:18, proto:19, state-update:2
  ...
  # was substrate-level: Rushwick-content carries color as a ward-level feed property.
  # c09 s02 is particular-level: a specific courier body, at a specific gate, in a specific
  # delivery posture, at a wrong hour — color sharpens from substrate-ambient to directional
  # inference on a named particular. The resentment-register is the same continuous
...
6 @23 actor:taylor-hebert-kl-122ac ++ atonement-as-repetition: [double-omission-sealed-in-wax-not-named, seal-as-form-fitting-before-recognition, two-substrates-one-surface-confirmed-by-wax] | licensed-by: proto:23, proto:25, proto:26, proto:27
  ...
  # seal drying is the atonement-pattern in its most material form so far: the form fits
  # (the packet holds exactly what it holds), the recognition has not arrived, and the
  # hardening wax confirms the structure without Taylor naming the structure. New
  # event-frame: prior atonement-as-repetition tokens cover routing-forms and integration-
  # events; this token covers the sealing-act itself as form-completion-before-legibility.
  # No string overlap with existing bundle.
...
7 episode + double-omission-architecture: [two-records-one-surface-neither-requiring-naming, withholding-enacted-through-substrate-selection, double-omission-as-structural-fact-not-volitional-act, silence-as-the-chapter-discipline] | licensed-by: proto:8, proto:10, proto:19, proto:24, proto:25, proto:26, proto:27
  ...
  # map, while the deliverable receives adjacent content; the architecture itself holds the
  # split without Taylor naming the gap. The episode-scope vibe biases all operators for
  # b01c09 rendering toward the operational-discipline register: no interior weighing,
  # no recognition beat, no named withholding. The substrate split IS the chapter's
  # discipline and silence is its surface. New episode-scope keyword (not pre-seeded).
