facet: state-updates-taylor-hebert-kl-122ac
episode: b01-c15
author: impersonator(taylor-hebert-kl-122ac)
---

# rubric-carve-out — POV actor-state co-citation: capability/coverage-scope advance anchored at the competency-in-use beat, not the establishing-anchor beat
#
# rubric-state-updates.md (design/shoot-v2/rubric-state-updates.md) § Cross-facet test "POV-character actor-state shifts require narrator-interest co-citation"
#
# Carve-out scope: state:1 (capability/coverage-scope advance)
# Carve-out rule: the scene-map ties the capability gain-side to @1 (ward anchored at the Red Keep outer margin) + @16 (arch-fly density read). Neither @1 nor @16 carries a narrator-interest fire (both are establishing/feed-mechanic beats, correctly silent on attention). The advance is co-cited at @17 instead — the first beat where the two-month-learned competency is in load-bearing use AND narrator-interest fires on it (the bay-warm/hill-warm differential read).
# Coverage justification: the field genuinely flips when the competency does load-bearing work (the thermal-differential read that confirms the non-extractable channel), not at the silent establishing-anchor; firing at @17 satisfies the POV co-citation contract without inflating a narrator-interest fire onto a feed-mechanic beat.
#
