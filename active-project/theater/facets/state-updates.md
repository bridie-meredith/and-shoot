...
# FLAGGED for cut by cycle-2 — cape-fic-reader correct: entry 10 places the discipline-hold at @8 (ambient-passive -> threshold-held-against-density-spike) with no released-from-hold transition between @8 and @12; passive insect data continues to arrive but Taylor's deliberate conversion of that data into canonical knowledge-state should not fire while the hold is active; the `passively` qualifier does not resolve this because the hold is precisely against deliberate passive-data processing. CUT: knowledge.hook-block-density-map entry for this chapter; Taylor receives the density data but it does not enter canonical knowledge-state under the discipline-hold; if a density-map entry is needed it should fire at a post-hold or post-chapter beat. [cycle-2 fixer, 2026-05-19]
