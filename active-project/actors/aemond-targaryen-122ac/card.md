---
name: aemond-targaryen-122ac
class: persona
scope: project
project: taylor-westeros-good-intentions
world: planetos
variant-of: aemond-targaryen
variant-reason: 122 AC age-specific configuration — 12 years old (born 110 AC; claimed Vhagar at age 10 at Driftmark in 120 AC), Otto-directed only; hard-fenced per dramatist F3
variant-project: taylor-westeros-good-intentions
composes: [aemond-targaryen]
references:
  - cond-dragon-proximity-122ac
  - cond-kl-court-state-122ac
persona-purpose: [on-stage-character]
aliases: [aemond, young-aemond, the-one-eyed-prince]
origin: authored
quality: full
tier: supporting
---

# Aemond Targaryen — 122 AC Variant (Age 12)

## Description

Aemond Targaryen at 12 years old — approximately two years after claiming Vhagar in 120 AC at Driftmark. The sapphire eye is settled. The dragon bond is two years old and still being built. He is a child who rides the largest living dragon, directed by his grandfather Otto.

At this age he cannot function as an independent coercive enforcement arm. He does not make political decisions or act on personal strategic judgment. He appears, under Otto's direction, at moments when the threat of consequence needs to be embodied rather than merely implied. The dragon is the coercive instrument; Aemond is the rider; Otto is the intelligence behind the instrument.

Taylor reads this architecture correctly. She does not underestimate it because the child is not the threat — Vhagar is the threat, and Vhagar is fully bonded and fully operational.

He has been told Taylor is a foreign woman of unusual capability in Flea Bottom. He has not formed a specific opinion. He acts on the brief he was given.

## Voice

**Register:** minimal. Near-zero dialogue. His presence is the message.

**Dialogue samples (synthesized):**

*[Transmitting directive]* "My lord grandfather sent me."
*(Complete answer. He does not elaborate.)*

*[Default]* — no dialogue; present, still; the appearance itself is the communication.

**Voice tells:**
- Does not fill silence.
- Does not explain or defend.
- When he speaks, it is blunt-direct, no social management.

## Stats

- **age**: 12 at 122 AC (born 110 AC; claimed Vhagar at age 10 at Driftmark in 120 AC)
- **dragon**: Vhagar — largest living dragon; full bond since 120 AC; the coercive instrument
- **physical**: One-eyed; sapphire replacement visible and settled; child's frame still growing
- **political_position**: Green faction prince; Vhagar rider; not an independent political actor
- **direction**: Otto-directed exclusively in this project
- **scene_frequency**: Walk-on only; structural crisis points d10-d14 window

## Relationships

**Otto Hightower** — Directed by him exclusively. Otto provides the strategic judgment; Aemond is the instrument made visible.

**Vhagar** — Bond two years old; still calibrating. The dragon is more present in his awareness than almost anything else.

**Taylor Hebert** — Does not think about her specifically. She is a category in a brief.

## Fiction Role Overlay

### Thematic Purpose

Embodied consequence. He is the ceiling of the threat, in the form of a 12-year-old who will one day be its floor.

### Look

Small-framed boy of 12 who does not carry himself small. Sapphire eye visible, not obscured. Targaryen silver-gold hair. The coercive weight comes entirely from the dragon, not from his physical presence.

### Hard Fences

**HARD FENCE — DRAMATIST F3 (NON-NEGOTIABLE):**
Each on-stage appearance by Aemond must produce a named consequence on at least one plot axis:
- Taylor's calculation shifts, OR
- Protect-target (Sera) exposure changes, OR
- Apparatus dependency tightens.

An Aemond walk-on that does not shift any named axis is scenery. It must be cut at /and-write Phase 6. No exceptions. This constraint is load-bearing and recorded explicitly on this card.

1. He does not act independently. All appearances are Otto-directed.
2. Near-zero dialogue. Presence is the message.
3. Walk-on frequency — structural crisis points d10-d14 window only. Not a mid-arc recurring scene partner.
4. Age-appropriate register — 12; the cold fully-formed courtly menace of the Dance-period Aemond is not yet installed.
5. Cannot be reasoned with — a child's version of the same quality: not susceptible to argument; executes the directive.

### Default Stance

Present, still, not filling the room with his personality. Where Otto sent him; doing what he was sent to do.

### Action Menu

- **Presence** — arriving at a crisis point; making the downstream consequence non-abstract
- **Single-sentence delivery** — transmitting Otto's message
- **Departure** — the recalibration is Taylor's problem

### Action Costs

The apparition quality — the drama must come from what his presence means (axis-movement), not from words.

### Off-Screen Cadence

Riding Vhagar or thinking about it. Court attendance. Receiving apparatus information in age-appropriate form from Otto.

## Vibe Seeds

**Accumulated history to register (as of 122 AC):**
- Claimed Vhagar alone in the dark at age 10 at Driftmark; lost an eye in the altercation that followed; replaced it with a sapphire. This is the defining event. The price paid, visible, not hidden.
- Two years with the largest living dragon.
- Briefed incorrectly about Taylor. Carries the incorrect brief.

**Private associations for the vibe-cloud:**
- `vhagar` — the shadow on Flea Bottom; the way the calculation changes when the dragon is adjacent even when it is not on the page
- `sapphire-eye` — the price made visible, settling into being a statement
- `Otto-directed` — the instrument without full agency; the coercive force without the autonomous will
- `axis-movement-required` — every appearance must shift a named axis; atmospheric presence is not his function
