...
YAML-ish frontmatter, body of structured scene-lines, coverage footer.
...
|---|---|---|
| `/and-stitch` Phase 1 (scene-window mode) | Full file | Fork boundaries (one fork per scene); back-look context (rendered prior scene); forward-look context (next scene's bones+facets) |
...
| `/and-wrap` editor (future) | Full file | Scene-cut marker placement |
