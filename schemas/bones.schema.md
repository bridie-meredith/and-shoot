...
- **Subject must be a concrete actor, not an abstraction or apparatus (DEC-0115 / PROP-0046; 2026-06-08).** The named-entity license applies to *physical* subjects only — `the door`, `the yard`, `the body`. Apparatus and abstraction subjects — `the count`, `the gap`, `the feed`, `the node`, `the ledger`, `the categorization`, `the process`, `the intelligence-value` — are HARD `FAULT-FORM-ABSTRACTION-AS-SUBJECT`. The mechanism the POV narrator uses to perceive (a feed, a count, a ledger, an insect-network) is a *lens*, not an actor; it may not be the grammatical subject of a bone. Re-author with the concrete actor performing the physical act: `the count closes` → `taylor closes the ledger` or `taylor stops tracking him`; `the gap propagates` → `the three figures step back`. The stitcher cannot un-abstract what the bones never made concrete. Cross-reference: `/and-write` Phase 6 enforces this as `ABSTRACTION-AS-SUBJECT-<bone>` (HARD bone-gate finding); per-scene concreteness floor also enforced there.
