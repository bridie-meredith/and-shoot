...
- **Open proposal with same target + change_type** → do not duplicate. Append `recurrence_refs: + <new evidence ref>` to the existing entry and increment its `recurrence_count`. Return verdict `OK-MERGED` with `proposal_id: PROP-<NNNN>`.
