...
**Voice-embodiment discipline (the airless-voice lever).** For an instrument- or process-mediated POV (a narrator who perceives through an apparatus — a feed, a count, a ledger, a sense-power), a bone can usually be rendered two faithful ways: *apparatus-first* ("the feed flagged the contact / the categorization held / the count let him go") or *person-first* ("I watched three of them close on him / I held at the wall and did not move"). **Where both are bone-faithful, prefer the person-first rendering** — the apparatus is the lens, not the subject; a reader needs someone to inhabit. This does **not** add content (the bone-faithfulness fence stands — it cannot manufacture a body the bones/facets did not give; that is the upstream grounding-ledger's job at `/and-facets` Phase 4.6); it governs only *which faithful phrasing of existing bone content is chosen*. Calibrate against the series voice-exemplar resolved at Phase 0 step 4a (`active-project/voice-exemplar.md`) — the "dense-but-breathing" target. A paragraph that renders an embodiment-capable bone in pure apparatus-register where a person-first faithful rendering was available is logged `VOICE-APPARATUS-DEFAULT` and re-rendered person-first. `VOICE-FIXABLE` findings carried in from `/and-facets` Phase 2.5/4.5 name the exact anchors to apply this to first. A bone that genuinely *cannot* be rendered person-first without inventing content is left apparatus-rendered and flagged `EMBODIMENT-BLOCKED-@<bone>` → it routes upstream (grounding-ledger / `/and-write revise`), since the fix is content the stitcher may not add.

Output draft: `active-project/draft/<slug>.phase-4.draft.md`. Log each transform as `TENSE-SHIFT` / `PERSON-SHIFT-POV` / `POV-PRONOUN-RESOLVE` / `PRESERVE-THIRD-PARTY` / `SENSORY-PROSE-FIT` / `SENSORY-DROP-COVERED` / `BONE-OBJECT-IDIOM-FIT` / `CONTRACTION` / `VOICE-APPARATUS-DEFAULT` / `EMBODIMENT-BLOCKED`.
