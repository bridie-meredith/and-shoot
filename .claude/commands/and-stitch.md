...
The exposition facet is authored upstream at `/and-facets` time by the `exposition-author` subagent. Phase 0.6 reads that facet and assembles the preamble. **This phase does NOT dispatch an Agent call** — exposition is graph-resident, audience-modeled, source-cited, and mechanically audited (the `/and-facets` Phase 4 auditor; the R2 round + audience-gate are retired under DEC-0116). The stitcher's job is to **consume** it, not re-author it. **Note (PROP-0004 / DEC-0116):** exposition entries default to `surface: reference` — consult them for world-grounding but do NOT fold inline as prose unless the entry carries a `surface: render` + `licensed-context-exception` token (a context-ledger-licensed orientation add). The inline em-dash gloss fold-in is off by default.
...
- **Reference-only entries (PROP-0004 / DEC-0116)**: entries with `surface: reference` are consult-only — load them for world-grounding context but do NOT fold them into the rendered preamble or anchor positions. Only entries with `surface: render` (or, for legacy pre-PROP-0004 chapters, no `surface` field at all) enter the render pools above. A `surface: render` entry will carry a `licensed-context-exception` token (it was promoted from reference by a `/and-facets` Phase 3 context-ledger license).
- **Refused/dropped entries**: if any entry has a `refused: <reason>` field or has been DELETED (gap-line in monotonic ID sequence with `# DELETED` comment, written by the `/and-facets` Phase 4 fixer on HARD remediation — the R2 judge that used to write these is retired under DEC-0116), skip — do not render. The DELETED marker is the canonical authority.
...
**Cross-episode register check (informational only).** The fork can check `active-project/staff/exposition-author/glossed-terms.md` to see which terms were glossed in prior episodes. The `/and-facets` Phase 4 auditor's CONSTRAINT re-gloss check should have already caught re-gloss entries (the R2 exposition judge that used to cull them is retired under DEC-0116); if one slips through, the fork emits a `WARN-EXPOSITION-REGLOSS` to the render-log but still renders (the warning is for the next-episode auditor to catch).
...
- **Apply the voice-embodiment discipline (URI-STITCH-VOICE-EMBODIMENT — PROP-0022; 2026-05-29)** — see below.

**No-ledger discipline (DEC-0115 / PROP-0046–0047 — supersedes the prior "person-first preference").** For an instrument- or process-mediated POV (a narrator who perceives through an apparatus — a feed, a count, a ledger, a column, an insect-network, a sense-power), the apparatus is a **lens, never the subject of narration and never the unit by which events are reported.** Ledger/accounting/apparatus register is **PROHIBITED**, not merely de-preferred. *Apparatus-first* renderings — "the feed flagged the contact," "the categorization held," "the count let him go," "I closed the entry," "the gap propagated," "I held the feet," "the node foreclosed" — are FAILS, not stylistic choices. Render the **concrete physical/human action** the bone encodes: "I sent the flies across their hands and the nearest dozen stepped back," "they took him down the corridor and the lane went quiet behind them," "I kept still against the wall." Coldness, control, and analytical distance survive as *character* (what the narrator notices, how little she lets herself feel), not as bookkeeping vocabulary. This does **not** add content (the bone-faithfulness fence stands — it cannot manufacture a body the bones/facets did not give); it governs *which faithful phrasing of existing bone content is chosen*, and bans the abstracted one. Calibrate against the series voice-exemplar resolved at Phase 0 step 4a (`active-project/voice-exemplar.md`) — the "dense-but-breathing" target (concrete, embodied, fully legible). A paragraph rendered in ledger/apparatus register is logged `LEDGER-REGISTER-@<bone>` and **re-rendered concrete** (HARD within-stitch — it may not ship). A bone that genuinely *cannot* be rendered concretely without inventing content (because the bone itself is an abstraction-as-subject) is flagged `EMBODIMENT-BLOCKED-@<bone>` → and when a chapter accumulates `>= K` (default K=3) such flags it **routes upstream to `/and-write revise`** (the fix is concrete-event content the bones must supply; see PROP-0049 concreteness floor), rather than shipping apparatus-rendered. `VOICE-FIXABLE` findings carried in from `/and-facets` Phase 2.5/4.5 name the exact anchors to apply this to first.

Output draft: `active-project/draft/<slug>.phase-4.draft.md`. Log each transform as `TENSE-SHIFT` / `PERSON-SHIFT-POV` / `POV-PRONOUN-RESOLVE` / `PRESERVE-THIRD-PARTY` / `SENSORY-PROSE-FIT` / `SENSORY-DROP-COVERED` / `BONE-OBJECT-IDIOM-FIT` / `CONTRACTION` / `LEDGER-REGISTER` / `EMBODIMENT-BLOCKED`.
...
- **Exposition-derived sentences (preamble paragraphs, scene-orient bridges, fold-in glosses): apply Q9 (anti-jargon) and Q6 (fancy punctuation) normally; treat Q1 (load-bearing) as pre-cleared by upstream audience-modeling and Q5 (hollow-prose) / Q8 (asinine) as pre-cleared by the `/and-facets` Phase 4 mechanical audit (the R2 round is retired under DEC-0116; the auditor is the facet-layer clearance). Borderline Q1/Q5/Q8 on exposition-derived prose = KEEP (the audience-gap is the load-bearing claim; second-guessing it at Phase 7 invalidates the upstream gap-test). Q9 + Q6 still cut/reword normally — a Q9 jargon-hit on exposition is a fault that should have been caught at the audit stage; surface as `FAULT-EXPOSITION-AUDIT-MISS` and REWORD inline.**
- **Dialogue-derived sentences (the utterance itself, NOT the surrounding attribution clause): treat Q1 / Q5 / Q8 / Q9 / Q6 as ALL pre-cleared by the dialogue-writer fork's behavior-card-anchored authoring (at `/and-write`) plus the `/and-facets` Phase 4 auditor's dialogue card-compliance + Earth-Bet-fence checks (the audience-gate is retired under DEC-0116). Borderline = KEEP. A Q9 jargon-hit on an utterance is `FAULT-DIALOGUE-AUDIT-MISS` and surfaces for re-author at `/and-write revise` (dialogue is authored upstream); the stitcher does NOT REWORD utterances (the verbatim invariant from Phase 1 holds through Phase 7). The attribution clause (`he said`, `she answered`) IS subject to all of Q1–Q9 normally — Phase 7 may cut a redundant attribution (`Q1=no → CUT`) but cannot touch the utterance it attributed.**
...
**NAIVE-FOLLOW sub-gate (DEC-0115 / PROP-0047 — `FOLLOW-FAIL`, blocking).** The bar is not "could a determined reader eventually reconstruct events" — it is "did the prose *plainly render* what physically happens." Fail the terminal gate as `FOLLOW-FAIL` (routes to `/and-write revise`) when any of:
- Answer 1 reports a stretch with "no event I can name" that the contract says contains an event (the prose buried a real event in register).
- Answer 3 (causality) or the reader's own confusion notes flag that they could not tell what physically happened, or that events are narrated as abstraction/process/bookkeeping rather than concrete action (e.g. the reader paraphrases the chapter as "someone does accounting" or cannot say *who did what to whom*).
- Answer 6 (one-line summary) describes the chapter's *register/mood* instead of its *events* because the events were not legible.
- **Reviewer-excuse ban.** The diff harness may **NOT** discount a confusion report on the grounds that the cold register is the intended signature. Per DEC-0115 the ledger/apparatus register is retired; opacity is a defect, never a license. "Design-inherent" is not an available disposition for a `FOLLOW-FAIL` (and is consecutive-capped at N=2 even where it is available — see Step 4 / PROP-0048).
...
**Separated scoring — two axes (PROP-0022 / URI-READABILITY-TWIN; 2026-05-29).** The verdict scores the assembled draft on TWO independent axes; a clean PASS requires BOTH to pass — a chapter cannot earn terminal by being maximally complete while reading airless (the c05 failure):
- **Completeness axis** — could the cold-reader recover the central event / jeopardy / causality (the Step 2 diff), and is the cluster/prose-rationale set clean? (The existing machinery above.)
- **Readability axis** — from the cold read's answers, is there *a person to follow* and does the prose *breathe*, or is it apparatus reporting itself? Derive from: the cold-reader's CONTINUE rationale + any "couldn't find anyone / dense / abstract" complaint, AND `VOICE-APPARATUS-DEFAULT` / `EMBODIMENT-BLOCKED` counts from Phase 4, AND a `spine-staging-gap`/airless cluster. Classify `READABLE` / `AIRLESS`.
The two axis-results compose: **PASS** requires completeness-pass AND `READABLE`. `AIRLESS` with completeness-pass → at least `PASS-WITH-DEPTH-PASS-REQUIRED` (the depth pass is the de-abstraction render-pass + any grounding-ledger adds); `AIRLESS` on the central event itself → `FAIL` (route to `/and-write revise` for grounding + re-stitch with the voice-embodiment discipline). Record `chapters[<slug>].cold_read.readability_axis = {verdict: READABLE|AIRLESS, basis: <one line>}`.

**Consecutive-caveat circuit breaker (PROP-0048 / CLAUDE.md Rule 22).** Before emitting the verdict, read `active-project/staff/showrunner/aggregate-state.md` `caveat_history[]` (if the file exists). For any defect class that this chapter's cold-read would accept under "design-inherent / accepted-caveat" — i.e., the chapter is heading toward PASS-WITH-DEPTH-PASS-REQUIRED and the cluster/finding is not a hard decomposition failure but a recurring pattern being tolerated — look up that class in `caveat_history[]`. If `consecutive_count >= 2` (N=2, the default per PROP-0048), the standing caveat disposition is **EXHAUSTED**: auto-promote the verdict from PASS-WITH-DEPTH-PASS-REQUIRED to **FAIL-CONSECUTIVE-CAVEAT** with routing to `/and-write revise`. "Design-inherent" is no longer renewable after N=2 consecutive chapters. Print:
```
/and-stitch Phase 9 FAIL-CONSECUTIVE-CAVEAT — <chapter> blocked by circuit breaker.
Defect class: <class> has been dispositioned design-inherent for <consecutive_count> consecutive chapters (max N=2).
This is a systemic defect, not an acceptable signature. Depth-pass or principal escalation required before further ships.
next: /and-write <chapter> revise   (then re-cascade /and-facets + /and-stitch)
```
If `aggregate-state.md` does not yet exist (pre-c01 or no Phase 10 has run), skip the check — no history to evaluate. If the circuit breaker fires, Phase 10 is skipped (same as a normal Phase 9 FAIL).

- **PASS** — the chapter is terminal, no cluster present AND `prose_rationale_audit.verdict != SOFT-BLOCK` AND `readability_axis.verdict == READABLE`. Print the cold reader's one-line summary, the staging-signal count, and `next:`. If `staging_signals > 0` but no cluster, recommend `/and-write <chapter> revise` + re-cascade as an optional depth pass (non-blocking). **Depth-pass resolution stamp:** if `chapters[<slug>].depth_pass_pending == true` (set by `/and-write` Phase 7 in revise --from-signals after a prior PASS-WITH-DEPTH-PASS-REQUIRED), stamp `chapters[<slug>].depth_pass_resolved_at = <iso-timestamp>` and clear the pending flag. This confirms the depth pass delivered; downstream consumers (`/and-substance book` Phase 0, `/and-review verdict <book>`) treat the chapter as resolved.
...
- **Subsequent chapters**: read the existing file, then update:
  ...
  - **`caveat_history[]` (PROP-0048 circuit breaker).** Update based on this chapter's Phase 9 verdict:
    - If Phase 9 verdict was **PASS-WITH-DEPTH-PASS-REQUIRED**: identify the defect class(es) being tolerated (from `cold_read.signal_clusters[]` pattern labels and/or `cold_read.prose_rationale_audit.verdict == SOFT-BLOCK`). For each class: if it already exists in `caveat_history[]` AND `last_chapter` is the immediately preceding chapter (i.e., consecutive), increment `consecutive_count` by 1. Otherwise, set `consecutive_count = 1`. Set `last_chapter = <chapter-slug>`. Append new classes; update existing. Tag `last_updated_by: and-stitch-phase-10`.
    - If Phase 9 verdict was clean **PASS** (no caveats accepted): for each class currently in `caveat_history[]`, if the class had no corresponding cluster/finding in this chapter's cold-read, remove the entry (streak broken; count of zero is invalid per schema validation rule §9). If the class DID appear but was not raised to PASS-WITH-DEPTH-PASS-REQUIRED level (i.e., it was a SIGNAL that didn't cluster), leave the entry unchanged — the streak only resets on a clean chapter.
    - If Phase 9 verdict was **FAIL** or **FAIL-CONSECUTIVE-CAVEAT**: Phase 10 is skipped; no caveat_history update.
