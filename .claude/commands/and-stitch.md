...
**Consecutive-caveat circuit breaker (PROP-0048 / CLAUDE.md Rule 22; N=2).** After scoring the two axes but before printing the verdict, read `active-project/staff/showrunner/aggregate-state.md`. Check `caveat_streaks[]` for any entry with `defect_class: AIRLESS` and `consecutive_count >= 2`. If this chapter's `readability_axis.verdict == AIRLESS` AND the streak is ≥ 2: the AIRLESS finding may no longer be accepted as a NOTE — this is the circuit-breaker chapter (N+1):
- If the verdict would be clean PASS (completeness-PASS + AIRLESS treated as note): **auto-promote to `PASS-WITH-DEPTH-PASS-REQUIRED`**. Set `chapters[<slug>].cold_read.circuit_breaker_fired: true`. Print:
  ```
  CIRCUIT BREAKER (Rule 22 / PROP-0048) — AIRLESS streak at <N> consecutive chapters. "Design-inherent" is no longer an available disposition for this defect class. Verdict promoted from PASS → PASS-WITH-DEPTH-PASS-REQUIRED (mandatory depth pass).
  ```
- If the verdict would be PASS-WITH-DEPTH-PASS-REQUIRED already: no change to verdict, but set `circuit_breaker_fired: true` so the depth pass is clearly flagged as circuit-breaker-mandated.
- FOLLOW-FAIL is always a hard FAIL per Step 2; the circuit breaker does not apply.

**After the verdict, update `aggregate-state.md.caveat_streaks[AIRLESS]`** (this update is part of the Phase 10 aggregate-state write; include it in the Phase 10 emit):
- Chapter shipped with `readability_axis: AIRLESS` as accepted caveat (PASS or PASS-WITH-DEPTH-PASS-REQUIRED without circuit-breaker reset): increment `consecutive_count`, append this chapter slug to `chapters[]`, update `last_chapter`.
- Chapter is `READABLE` or is a FAIL: reset the streak (`consecutive_count: 0`, `chapters: []`).
- Circuit breaker fired: stamp `circuit_breaker_fired_at: <this chapter>` in the entry, then reset the streak as above.
