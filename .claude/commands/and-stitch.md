...
**Consecutive-caveats counter maintenance (PROP-0037 companion; 2026-06-23).** In the same write, maintain `books[<book>].consecutive_shipped_with_caveats`: **increment by 1** when this chapter's verdict is `PASS-WITH-DEPTH-PASS-REQUIRED` (the SHIPPED-WITH-CAVEATS class — chapter ships but carries a deferred-quality caveat); **reset to 0** on a clean `PASS`. (On `FAIL` the chapter does not ship — Phase 10 is skipped and no counter update happens until it re-passes.) This counter is read by `/and-substance chapter` Phase 0 step 6.5, which HARD-aborts the next chapter's production run at `>= 3` consecutive unless a `cohere_acknowledgment` is stamped. Schema: `books[<book>].consecutive_shipped_with_caveats` (integer, default 0) per `schemas/showrunner-memory.schema.md`.
