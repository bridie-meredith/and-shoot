...
11. **PILE-UP REVIEW** — proto-lines with >4 co-located facets; verdict per pile-up: warranted | over-decoration. (With no R2 density-judge, PILE-UP is the auditor's over-decoration check — flag over-decoration HARD-to-fixer when a pile-up fails the warranted test.) **Warranted test (both required):** (a) the anchor is listed in the scene's `peak-bones` or `peak-shadow-bones` in the scene-map, OR the scene's `rhythm-shape` is `rising-to-peak`, `peak-and-release`, or `double-peak`; AND (b) every co-located facet contributes to a distinct character × facet-class pair — no same-character same-class redundancy across the pile-up. **Over-decoration when any fires:** scene is `flat-low`, `resolving`, or `release-only`; OR any character × facet-class pair appears in ≥2 co-located entries at the anchor.
