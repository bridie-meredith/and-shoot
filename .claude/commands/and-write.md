...
- **Abstraction-as-subject check (URI-WRITE-EVENT-CONCRETENESS + DEC-0115 / PROP-0049 — HARD; 2026-06-08).** Extends the central-event concreteness check to *every* bone, not just the spine. A bone whose grammatical subject is the apparatus or an abstraction rather than a concrete actor — `the count closes`, `the gap propagates`, `the node forecloses`, `the feed returns the body`, `the ledger takes the entry` — is `ABSTRACTION-AS-SUBJECT-<bone>` (HARD). Re-author with a concrete actor + concrete action (note `I close the count` is still apparatus-vocabulary → prefer `I stop tracking him`; `the three figures strike the courier` passes). Per DEC-0115 the ledger/apparatus register is retired at the *bone* layer — the stitcher cannot un-abstract what the bones never made concrete, so the fence lives here at origin.
- **Scene concreteness floor (URI-WRITE-EVENT-CONCRETENESS + DEC-0115 / PROP-0049 — HARD; 2026-06-08, replaces the prior SIGNAL).** If grounding-class bones number fewer than `ceil(0.6 × (bone_count − chatter_count))` in a scene, fire `SCENE-ABSTRACT-DOMINANT` (HARD — blocks emission): the scene is carried by perception/process/bookkeeping verbs and will read airless and event-poor downstream. Remediate by adding grounding bones / re-authoring abstract bones concrete. **There is no accept-with-rationale disposition** — the former `ABSTRACTION-DOMINANT` SIGNAL's "stylized instrument-POV intentional" escape was the standing excuse that certified the b01 airless cohort chapter after chapter (16 consecutive design-inherent dispositions, DEC-0060..0104). "Instrument-POV" is rendered through *concrete perception of concrete events*, not through abstraction-as-narration.
...
| finding | severity |
|---|---|
| flat-bone, cost-not-paid, missing-opposing-force, per-axis-Δ-mismatch beyond ±2, SUBSTANCE-FLAT, SUBSTANCE-SUSPECT, EVENT-UNCOVERED, EVENT-MAP-INCOMPLETE, STAKES-AXIS-NOT-DOMINANT, AXIS-UNDERDELIVERED, SENSORY-GROUNDING-ABSENT, **EVENT-NOT-CONCRETE**, **ABSTRACTION-AS-SUBJECT (any bone; DEC-0115)**, **SCENE-ABSTRACT-DOMINANT (grounding bones < 60% of non-chatter bones; DEC-0115)**, **FAULT-DIALOGUE-MISSING-AT-ANCHOR, FAULT-DIALOGUE-CARD-VIOLATION, FAULT-DIALOGUE-OBJECTIVE-MISSING, FAULT-DIALOGUE-EARTH-BET-FENCE** | HARD (blocks emission) |
