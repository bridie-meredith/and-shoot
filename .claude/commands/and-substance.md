...
| auditor | Does the chunk text match the substance contract? No rank claim without described cause. Cost-ledger consistency. Per `schemas/audit-report.schema.md`. **Thematic-axis-coverage (chapter level, URI-CONTRACT-THEMATIC-AXIS):** the chapter `goal` names a thesis; the contract must declare that thesis axis in `axes_in_motion[]` or `axes_held[]`. A `goal` about a moral-framework turn whose contract never lists `moral-framework` is under-declaring its own thesis — `THEMATIC-AXIS-UNDECLARED-<axis>`, HARD at this level (blocks persist, forces revise). The mechanical sum/enum checks never catch this; it asks whether the contract is about what the chapter is about. **Series level additionally — signature register-floor (PROP-0050 / CLAUDE.md Rule 22, 2026-06-08):** the accepted series signature MUST declare an explicit readability/concreteness floor as a non-negotiable constraint coexisting with the register (e.g., `readability_floor: {concreteness_bone_ratio: ">=0.6", naive_follow_required: true}`). A signature that names only state-axes and a prose register without such a floor is satisfiable by register manipulation alone — fire `SIGNATURE-REGISTER-ONLY` (HARD; blocks persist, forces signature revise at Phase 4c). The floor is a non-negotiable constraint; the register is how the story feels *within* that constraint. |
