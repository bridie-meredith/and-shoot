...
- **Step 4a — Screen-writer proposes.** Dispatch screen-writer with `project.brief` + `project.constraints` + `series.chunk.prose` + `series.chunk.path` + `series.chunk.trajectory` + `series.structure.*`. For each universal axis (~9: wealth / health / community / emotional / capability / knowledge / reputation / agency / trust), screen-writer:
  ...
  - **Authors `readability_floor` (REQUIRED; PROP-0050 / DEC-0115 / CLAUDE.md Rule 22).** A prose-agnostic, measurable concreteness/legibility constraint the register must coexist with. The floor must be independently verifiable — e.g. "≥ 60% of chapters must earn FOLLOW-OK at `/and-stitch` Phase 9 (naive reader recovers the central event without apparatus context)" or "every chapter must stage at least one concrete physical action per scene legible without register priming." A floor whose entire content is satisfiable by the register itself (e.g. "the register's density IS the substance floor") is REJECTED. This constraint exists precisely because a single-axis register-as-substance optimization is what produced the b01 readability failure.
...
| auditor | Does the chunk text match the substance contract? No rank claim without described cause. Cost-ledger consistency. Per `schemas/audit-report.schema.md`. **Thematic-axis-coverage (chapter level, URI-CONTRACT-THEMATIC-AXIS):** the chapter `goal` names a thesis; the contract must declare that thesis axis in `axes_in_motion[]` or `axes_held[]`. A `goal` about a moral-framework turn whose contract never lists `moral-framework` is under-declaring its own thesis — `THEMATIC-AXIS-UNDECLARED-<axis>`, HARD at this level (blocks persist, forces revise). The mechanical sum/enum checks never catch this; it asks whether the contract is about what the chapter is about. **Readability-floor gate (series level only; PROP-0050 / DEC-0115 / CLAUDE.md Rule 22):** fires only when this is a `series` invocation. Check `series.substance.readability_floor`: if absent or empty → `SIGNATURE-NO-READABILITY-FLOOR` HARD (blocks persist; forces revise). If present, apply the register-satisfiability test: does the stated floor amount to "the register itself is the floor" with no independently-measurable concreteness constraint? If yes → `SIGNATURE-REGISTER-ONLY-FLOOR` HARD (same routing). A floor passes when it names a concrete, prose-agnostic legibility threshold (e.g. a Phase 9 FOLLOW-OK rate, a per-scene concrete-action minimum) that would fail even if the register were otherwise well-formed. |
