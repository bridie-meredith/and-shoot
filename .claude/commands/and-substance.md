...
- **Step 4a — Screen-writer proposes.** Dispatch screen-writer with `project.brief` + `project.constraints` + `series.chunk.prose` + `series.chunk.path` + `series.chunk.trajectory` + `series.structure.*`. For each universal axis (~9: wealth / health / community / emotional / capability / knowledge / reputation / agency / trust), screen-writer:
  ...
  - Drafts `readability_floor` — the signature's non-negotiable concreteness constraint (required per PROP-0050 / CLAUDE.md Rule 22): `min_concrete_svo_ratio` (number 0.0–1.0; default ≥ 0.6, matching the `/and-write` Phase 6 `SCENE-ABSTRACT-DOMINANT` threshold), `register_constraint` (one-line explicit statement of what prose register is prohibited, e.g. "apparatus-as-subject / ledger-accounting vocabulary disallowed"), `rationale` (one line). The signature may NOT be satisfiable by optimizing a single prose register — the readability_floor is the counterweight that coexists with the substance axes.
...
- **Step 4c — Persist on accept.** **Readability-floor validation (PROP-0050 / CLAUDE.md Rule 22):** Before persisting, verify `series.substance.readability_floor` contains all three fields with non-empty values: `min_concrete_svo_ratio` (number 0.0–1.0, must be ≥ 0.6 unless `rationale` justifies a lower floor), `register_constraint` (non-empty string naming the prohibited register), `rationale` (non-empty string). If absent or incomplete: emit `SIGNATURE-NO-READABILITY-FLOOR` (HARD) and re-prompt at Step 4b — do not persist until resolved. Once validated, edited YAML moves from `signature-draft.md` to `series.substance.*` in memory. Phase 5 review runs against the accepted signature. **NOTE:** `series.substance.actor_baselines[]` (per-actor positional grid, dense 8×9 matrix or similar per `schemas/showrunner-memory.schema.md`) is NOT authored at this step — the cast roster doesn't exist yet (cast is provisioned by `/and-cast` after `/and-substance series`). `actor_baselines[]` is authored later: see Step 4d below for the post-cast pass.
