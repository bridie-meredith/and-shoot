# SUPERSEDED by showrunner memory chapters[b01c18].scenes[] (persisted 2026-06-05T02:30:00Z, /and-substance chapter b01c18 Phase 6). Auditor fault-001/002 note corrections applied pre-persist. Archival copy.

# b01c18 scenes draft — /and-substance chapter Phase 2-3
...
### (a) Axes_in_motion scene anchors

- moral_framework DOWN 1.0 (cl02): **s02** — the full-coverage network activation crossing the
  irrevocable-density threshold; body-count runs past the prior number; calibrated qualifier
  removed as a structural change (the architecture now covers bodies in counts that cannot be
  described as calibrated; the act is enacted physically through simultaneous full-node activation).

- political_register-prot UP 2.0 (cl06): split **s03 +1.5** (court apparatus read at scale through
  compound eyes; concrete feed-bodies and their specific patterns; contempt near-saturation) +
  **s05 +0.5** (accounting close; contempt filed as ledger-entry without exit mechanism; cl06 fully
  landed).

- position-world UP 1.0 (cl07b): **s04** — succession document confirmed moved; Green faction holds
  the Small Council access window Taylor's intelligence picture named; direct product of the
  fortnight deployment.

- political_register-world UP 1.0 (cl07c): **s04** — same succession-move confirmation event;
  political_register-world rises as the direct product of the irrevocable deployment.

- position-prot-collapse DOWN 1.0 (cl07b): split **s02 -0.5** (deployment-activation; more load-
  bearing = more precisely disposable) + **s05 -0.5** (accounting close; disposal calculus visible
  in the drawdown accounting).

- social_tether-prot-collapse DOWN 1.0 (cl07a): split **s02 -0.5** (deployment at irrevocable scale;
  tether under maximum structural strain) + **s05 -0.5** (accounting close; last act before removal
  calculus completes).

### (b) Roll-up verification against chapter contract (±1 tolerance)

Chapter contract sum vs. scene authoring sum:

| axis | contract | scenes sum | within ±1? |
|---|---|---|---|
| moral_framework DOWN | 1.0 | 1.0 (s02) | EXACT |
| political_register-prot UP | 2.0 | 2.0 (s03 1.5 + s05 0.5) | EXACT |
| position-world UP | 1.0 | 1.0 (s04) | EXACT |
| political_register-world UP | 1.0 | 1.0 (s04) | EXACT |
| position-prot-collapse DOWN | 1.0 | 1.0 (s02 0.5 + s05 0.5) | EXACT |
| social_tether-prot-collapse DOWN | 1.0 | 1.0 (s02 0.5 + s05 0.5) | EXACT |

All 6 axes_in_motion sum to chapter contract. Zero axes exceed ±1 tolerance.

### (c) cl-d11 +0.5 cost-forward-carry-to-d14 disposition

The cl-d11 2nd tranche (+0.5 relational_anchor_status remaining from c17's cl04 arc) is NOT
settled in c18. Wren is screened throughout the deployment and her gap-lane is held blank even
at maximum-density coverage, which is structurally significant but does not constitute a
recognition event that moves the relational_anchor axis. The +0.5 carry is documented explicitly
in the axes_held rationale for relational_anchor_status on s01, s02, s03, and s05, confirming
the discipline at every scene where the axis appears. Disposition: cost-forward carry to d14
collapse settlement, where the un-priced relational item the calculus comes for settles the
remaining tranche.

### (d) moral_framework collapse enacted physically

The moral_framework -1.0 in s02 is enacted as a concrete structural change to the architecture,
not Taylor-interiority about it. The specific physical act: Taylor opens all network nodes
simultaneously — every feed-node across all five wards and the Flea Bottom approaches and the
Red Keep outer ring active at once, for the first time, in contrast to the ward-by-ward,
one-cluster-at-a-time deployment mode that has been the operative mode since c12. The body-count
crosses the number she last reached before this city — a concrete quantity, not an adjective.
The east-of-water-gate gap-lane sits blank in the middle of the full-coverage field: the one
lane that is not Khepri, surrounded by everything that is. The calibrated qualifier is removed
as a structural fact: the coverage density cannot be described as calibrated when the network
runs in this mode, at this count. The enactment is in the act of opening the nodes and the
count running past the threshold number, not in any interior accounting Taylor runs about it.

---

## Scene arc summary

s01 — The ask arrives: Otto's full-coverage deployment request delivered through the established
cipher channel; Taylor reads the scope notation and names to herself what complying will cost
without yet executing it. The precipice.

s02 — Taylor executes the deployment: full-coverage network open at maximum density, all nodes
simultaneously, the body-count crossing the prior threshold number; the moral_framework
irrevocable breach enacted as a concrete structural act; Wren's gap-lane held blank in the middle
of the maximum-density field; collapse axes advance.

s03 — Court apparatus read at scale: six days of full-coverage returns the court machinery in its
entirety through compound eyes — specific bodies performing specific factional functions; contempt
advances to near-saturation as the feed delivers not specimens but the system entire.

s04 — Green succession move lands: intelligence product delivered, confirmation received, the
succession document through the Small Council access window; position-world and
political_register-world advance as direct products of the irrevocable deployment.

s05 — The irrevocable named in the accounting: the network stands down ward by ward; the accounting
closes cleanly; the calibrated qualifier is gone in the record; contempt filed without exit
mechanism; suppression holds; blank column held again.
