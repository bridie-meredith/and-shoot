...
2. **Cross-facet integrity check.** State-updates is the consumer-side validator for tensometer's @64 strong-expect (irreversible registrations get co-cited) and @39 forbidden (held-against-turn class is registration-only, not canonical change). It is also the consumer-side validator for narrator-interest's POV-restriction: an actor-state shift on the POV character requires a narrator-interest co-citation; state shifts on other actors and on environment do not.
...
- Cross-facet honor: tensometer @64-class beats (irreversible registration) almost always carry a state-update; narrator-interest fires on actor:POV.knowledge.* shifts.
...
- **Held-against-turn (tensometer @39 class).** Beats where someone holds a charged position without resolving — *taylor sets her feet on the dirt where his next pace commits* (@39) — these are registration / body-charge, not canonical state-change. Tensometer's locked file explicitly forbids canonical state-update co-citation at @39. Honor.
...
- **Pre-empting a future beat.** Writing the @64 parallel-marks state-update at @63. The change has not occurred yet at @63; @63 is approach (tensometer 2, pause-before-commit).
...
- **The cross-facet test.** Check tensometer and narrator-interest for the anchor beat:
  - If tensometer @beat is @39-class (held-against-turn / pure registration per the tensometer file's annotations), state-update REJECT (canonical state-change forbidden).
  - If tensometer @beat is @64-class (irreversible registration, strongly expects state-update co-citation per tensometer's cross-facet contract), state-update on the affected target is strongly EXPECTED; absence is a flag.
...
1. **Registration-as-state.** Writing a state-update for a perception or registration beat where no field actually changes. The dominant naive contamination — narrator-interest, tensometer, and audience-interest are all *registration* facets; only state-updates is structural-delta.
...
3. **Held-against-turn fire.** State-update on a tensometer @39-class beat. Tensometer's locked file forbids this; honor.
...
9. **Density-on-flat.** Firing on every motion-verb in the proto-line file. State-updates is sparse — irreversible turns and persistent shifts only. The flat 1-zones (e.g., @1–@22 approach in s01e01) are nearly silent; the confrontation cluster (@23–@48 + @57 + @64) holds most fires.
...
- **Density alignment with tensometer transitions and peaks.** State-updates concentrates around 2-rung and 3-rung clusters; the @1-flat approach zone is nearly silent. Ratio of fires-per-beat in non-1 zones should exceed ratio in 1-only stretches by at least 2×, ideally 3×.
...
- **At least one fire per scene-with-irreversible-event.** A scene that contains a tens=3 peak-rupture should usually carry a state-update on the peak beat or its immediate aftermath. Absence is a flag.
- **Approach-zone permitted-silent.** A long approach zone of tens=1 (e.g. s01e01 @1–@22) is permitted-silent; state changes in approach are usually establishing-state authored at the project-setup baseline, not at proto-line beats.
...
- **Cross-facet kickback.** If tensometer fires a 3-cluster with no state-update support, either the rubric is missing a target the author should have written, or tensometer's annotation is overstated. Flag both ways for cross-facet review.
...
State-updates is the consumer-side validator for tensometer and narrator-interest, and the producer-side input to canonical write-back.

### Anchor expectations (consumer side, inherited from upstream facets)

- **Tensometer (locked).** Per the tensometer cross-facet contract:
  - **@64 strongly expects co-citation.** "Irreversible registration" — the parallel-marks beat. State-update on `prop:district-ledger.parallel-marks` (or equivalent), `actor:taylor.knowledge.record-state` (POV-character knowledge of the record, requires narrator-interest co-citation per @64 narrator-interest fire), and `actor:taylor.administrative-status: child-or-ward -> provisional-labor-eligible` are all expected.
  - **@39 forbids canonical state-update co-citation.** Held-against-turn class. Actor-posture co-citation is permitted (e.g., `actor:taylor.posture: ` if persistent), but canonical state-change is not. The tensometer file's STATE-UPDATE NOTE on @39 is the authority.
  - **@38 (climax peak, body-charge + reversal-proximity).** Actor-posture co-citation permitted (the body-reach is a posture); the irreversible *commit* is in tension with @39's hold. Author convention: fire `actor:taylor.posture` once across @38–@39 if persistent; do not double-fire.
- **Narrator-interest (locked).** Per the narrator-interest cross-facet contract:
  - **POV-character actor-state shifts require co-citation.** Every `actor:taylor.knowledge.*`, `actor:taylor.mask-state`, `actor:taylor.exposure-state` entry must have a narrator-interest entry on the same `@<proto-line-id>`. Narrator-interest's @52 fire ("the count of allies in the yard drops to one") signals an interior-knowledge shift; if the matching state-update doesn't fire, the cross-facet contract is broken.
...
- Tensometer ratings. State-updates does not change tens. If a beat's state-update suggests higher charge than its tens scalar, the auditor flags for cross-facet review — but tens stays locked.
...
- **Author (split):**
  - **Studio.** Authors `studio.*` and `prop:*.*` entries. Loads: locked tensometer, locked narrator-interest, locked location-state, the proto-line file, the studio state schema, the relevant location/prop cards. Per-beat pass: walk the proto-lines, identify field-mutations on environment or props, write entries.
  - **Dialogue-writer fork (per character).** Authors `actor:<that-character>.*` entries. Loads: locked tensometer, locked narrator-interest, the proto-line file, the character's persona card + state schema + behavior pack (if present). For the POV character (Taylor): also loads the locked narrator-interest file as a co-citation check (every `actor:taylor.*` entry must have a `@<beat>` match in narrator-interest). Per-beat pass for the character's own state.
...
- **Ceiling defense.** If an author defends a FIRE that the reviewer would push to NONE, the burden is on the author to name (a) which field on which target changed, (b) the persistence past this beat, (c) the cross-facet contract slot the entry serves (tensometer co-citation expectation; narrator-interest co-citation if POV actor-state). A FIRE that survives ceiling defense should also pass the strip-test and the persistence-test.
...
- Not registration. Narrator-interest, tensometer, and audience-interest carry registration. State-updates carries the structural delta.
...
- Not posture-noting. Most posture is tensometer's territory (body-charge). Posture-as-state requires multi-beat persistence AND load-bearing on the next move.
