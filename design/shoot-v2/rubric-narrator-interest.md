...
11. **Apparatus-as-subject register (DEC-0115 — HARD).** A narrator-interest entry whose grammatical subject is the apparatus, the count, the feed, the network, or an abstraction derived from it is prohibited. *The count closes*, *the feed flags the contact*, *the gap propagates*, *the node forecloses*, *the tally comes up short* — all are REJECT. The subject of an interest-flag must be a concrete actor or a concrete perceived event; the apparatus is the lens, not the narrator. Re-author with the lens's output: *the steward passes without a second check* (count→person), *the flies shift at the doorframe — someone new just crossed the step* (feed→event). The NI voice-fidelity ACCEPT signatures for inventory-tell register (`cost-language`, `pre-calc`, `weight-in-the-body`) are not affected — those fire on a concrete subject. Apparatus-as-grammatical-subject is the prohibited pattern regardless of POV character. (DEC-0115, 2026-06-08 — ledger/apparatus register retired at every authoring surface; paired with `/and-write` Phase 6 `ABSTRACTION-AS-SUBJECT` bone-gate upstream and `/and-stitch` Phase 4 `LEDGER-REGISTER` downstream; this rubric entry is the facet-layer enforcement.)
