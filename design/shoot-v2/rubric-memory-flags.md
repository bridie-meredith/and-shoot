...
Status: **V3 locked 2026-05-21.** V2 was locked at end of Phase 1 (original tuning). V3 adds one clause derived from the b01c01 cycle-2 cap-burn on `mem:1 @9`: a feel-as-spine equivalence carve-out for held-discipline scenes where the chapter's substance lives in feeling rather than narrator-interest.

## V3 changes summary (2026-05-21)

- **Licensing-discipline axis / Narrator-interest co-citation ACCEPT signature** — new "feel-as-spine equivalence" carve-out. When the chapter's substance lives in feeling rather than narrator-interest (specifically: `dramatic_shape: hinge` AND scene's `stakes_axis` is in `axes_held[]`), a memory-flag entry MAY co-cite a feel-flag (`feel:N @<proto-line-id>`) as spine in place of narrator-interest. The held-discipline substance is somatic, not narrated. Promoted from `URI-FACETS-V3-FEEL-AS-SPINE`.
- **Licensing-discipline axis / Missing narrator-interest co-citation REJECT signature** — updated to honor the V3 carve-out: only reject if NEITHER narrator-interest is fired NOR the V3 carve-out conditions all hold.

Both V3 additions trace to the b01c01 cape-fic-reader + dark-fantasy-reader cycle-2 memory verdicts (`active-project/staff/audience/{cape-fic-reader,dark-fantasy-reader}/memory-r2-verdict.md`). The carve-out is **structurally compatible with the 2026-05-21 axis-bookkeeping split** in `schemas/showrunner-memory.schema.md` — held-discipline scenes are exactly the `axes_held[]` scenes the new schema makes mechanically checkable.
...
The rubric depends on the locked scene-map facet file (`theater/facets/scene-map-<book>-<chapter>.md`), the locked narrator-interest file, and the base + variant behavior cards' §"Memory monuments" sections. It does NOT depend on (and must not be folded into) the open V3 rubric work for tensometer.

The pressure-signal source for quiet-beat classification is `per-bone substance_delta.axis_moves.magnitude` as stored in showrunner memory, surfaced through the scene-map's `rhythm-shape` and `peak-bones` fields. Load the scene-map file to classify any bone's charge level: bones inside a `flat-low` or `resolving` rhythm zone are quiet-beat equivalent (formerly tens=1 or trailing tens=2); bones listed in `peak-bones` are peak equivalent (formerly tens=3); bones in a `rising` zone without `peak-bones` membership are mid-charge equivalent (formerly tens=2 rising edge).
...
3. **Quiet-beat anchor.** Memory and monument fire when the foreground tension is low enough for the interior to reach into accumulated weight. **Memory-flags concentrate in bones with `rhythm-shape: flat-low` or `rhythm-shape: resolving` (as read from the scene-map). They are forbidden by default in bones listed in the scene's `peak-bones` array except under explicit displacement-clamp construction.** This is the **inverse** of narrator-interest's pressure-signal-alignment — narrator-interest fires on `rising` zones and `peak-bones`; memory-flags fire in the gaps. Where narrator-interest reads forward (the next beat is about to commit), memory-flags reads backward and outward (this beat lights something already standing). The two facets together produce the stitcher's render-grammar: peaks render in full (narrator-interest), gaps render with weight (memory-flags).
...
ACCEPT signatures:
- **Quiet-beat anchor.** The fire's anchor bone is inside a scene with `rhythm-shape: flat-low` OR `rhythm-shape: resolving` for its scene window (as read from the scene-map). Bones listed in the scene's `peak-bones` array are forbidden by default; an exception requires explicit displacement-clamp construction AND a defensible argument that the monument lit *because* of the peak's discharge (narrator-interest carries the peak; memory-flags carries the resonance after).
- **Narrator-interest co-citation (with feel-as-spine equivalence for held-discipline scenes — V3, 2026-05-21).** Mandatory in the default case. Every memory-flag entry must have a narrator-interest fire on the *same* `@<proto-line-id>`. The narrator-interest entry is the spine; memory-flags is the spine's monument-content. A memory-flag without a co-cited narrator-interest entry is suspicious — the narrator should have registered the trigger before the memory fired.

  **Feel-as-spine equivalence carve-out (V3).** When the chapter's substance lives in **feeling** rather than narrator-interest, a memory-flag entry MAY co-cite a feel-flag (`feel:N @<proto-line-id>`) as spine in place of narrator-interest. The held-discipline substance is somatic, not narrated: the body holds, the prohibition enacts, the architecture is the shape the protagonist will not build. In held-discipline scenes the narrator-interest layer fires sparsely (the interior is held, not reaching forward) while feeling carries the load. **All four conditions must hold** for the carve-out to apply:
    1. Chapter `dramatic_shape: hinge` (per `chapters[].dramatic_shape` in showrunner memory).
    2. Scene's `scene_conflict.stakes_axis` resolves to an entry in the scene's `axes_held[]` list (per the 2026-05-21 axis-bookkeeping split in `schemas/showrunner-memory.schema.md`) — i.e. the held axis IS the stakes.
    3. A feel-flag fire exists on the same `@<proto-line-id>` as the memory-flag entry.
    4. The displacement-cue, audience-meaningfulness, functional-register, and multi-justification gates ALL still clear under the standard rubric.
  If any of (1)-(3) fails, the carve-out does not apply and the standard narrator-interest co-citation is required. The carve-out replaces only the spine; every other discipline gate (hard-fence on description, monument-trigger, displacement-discipline, audience-meaningfulness, functional-register, per-scene cap, doubled-register visibility) still applies unchanged.

  (URI-FACETS-V3-FEEL-AS-SPINE, 2026-05-21 — promoted from b01c01 cycle-2 cap-burn on `mem:1 @9` "the feet hold and the architecture stays the shape she will not build." That entry's substance-hinge lived in `feel:1 @9` "she sets her weight even on both feet" — the somatic discipline-enactment beat — rather than in narrator-interest, which was silent at @9 because Taylor's interior was held, not reaching forward. Three remediation paths blocked under V2: culling mem:1 forced single-register file-level fail; adding NI:@9 would breach the 25% density ceiling; rubric-authority-ruling was the documented "preferred resolution" but never materialized. The cap-burned at cycle-2. V3 codifies the held-discipline-scene feel-as-spine path. Structurally compatible with the axes_held bookkeeping split shipped the same date — held-discipline scenes are exactly the `axes_held[]` scenes the new schema makes mechanically checkable.)
...
- **Per-scene cap.** **At most one memory-flag entry per scene.** A scene with two or more memory-flag fires is over-firing the licensing layer for that scene; the stitcher cannot meaningfully gate metaphor / callback density across multiple licensed beats in a single scene. Scene boundaries follow the scene-map file's `@<start>-@<end>` ranges (derived from substance_delta in showrunner memory). Two co-fired entries on a *single bone* (the doubled-register exemplar pattern from earlier rubric drafts) are also forbidden under per-scene cap — one bone is a member of one scene, and the scene gets one fire.
- **Sparsity discipline.** The file fires on **1–5% of bones** for chapter-length corpora. The per-scene cap and both-meaningful criterion together pull the density floor down. Sparsity is *the* load-bearing rubric property. Far better to under-fire and miss a monument than over-fire and dissolve the licensing layer.
...
REJECT signatures:
- **Peak-bone fire without displacement-clamp argument.** A fire on a bone listed in the scene's `peak-bones` array where the interior is fully committed to the action and no displacement / clamp construction earns the resonance. Reject.
- **Missing spine (with feel-as-spine equivalence — V3, 2026-05-21).** A fire on a beat where narrator-interest is silent AND the V3 feel-as-spine carve-out conditions do not all hold (chapter `dramatic_shape != hinge`, OR scene's `stakes_axis` is not in `axes_held[]`, OR no feel-flag fire on the same `@<proto-line-id>`). The narrator-interest spine OR (held-discipline + feel-flag spine) must register the trigger; if neither does, the memory-flag is firing without spine. Reject or flag back to narrator-interest / feel author for missed-fire repair. When the V3 carve-out applies, audit reports must explicitly state which spine is in effect (`spine: narrator-interest @N` or `spine: feel @N (V3-carve-out)`) so the verdict trail is auditable.
...
- **The quiet-beat test.** Load the scene-map file (`theater/facets/scene-map-<book>-<chapter>.md`). Check the bone's scene window. If `rhythm-shape: flat-low` or `rhythm-shape: resolving` — default-licit. If `rhythm-shape: release-only` — defensible with cue. If `rhythm-shape: rising` (bone not in `peak-bones`) — contested; require explicit argument that the monument is reaching backward, not forward. If the bone is listed in the scene's `peak-bones` array — default-forbidden; require explicit displacement-clamp construction and resonance-not-action argument.
- **The narrator-interest spine test (with V3 feel-as-spine equivalence).** Locate the beat in the locked narrator-interest file. If fired, the memory-flag entry must be consistent with what the narrator-interest entry registers — they cannot contradict on which channel/trigger lit. If silent, apply the V3 carve-out: check that chapter `dramatic_shape: hinge`, scene's `stakes_axis` is in `axes_held[]`, AND a feel-flag fires on the same `@<proto-line-id>`. If all three hold, the feel-flag is the spine and the memory-flag entry must be consistent with the feel-flag's somatic register. If any condition fails, REJECT or flag back to narrator-interest author for missed-fire repair.
...
- **The monument-card resolution test (target-reference must resolve via margit).** Every memory-flag entry's `target-reference:` field must name a monument card slug that resolves in the card library via margit referral (`cards/conditions/monument-*.card.md` or equivalent monument-class card). A bare gloss text in `target-reference` ("administrative-observation-apparatus displacement", "override-architecture-prohibition") without a margit-resolved slug fails the licensing-discipline axis: the monument is asserted, not anchored. If the monument is novel to the project, the author must margit-refer for card creation before the memory-flag fires. **SIGNAL** if the gloss is structurally clear and the card is queued; **HARD** if the gloss is opaque and no margit referral exists. (URI-FACETS-CYCLE-1, 2026-05-19 — promoted from audience-gate cycle-1 attack on b01c01 mem:2 @23: all three reviewers independently flagged the monument-card-resolution gap the mechanical scan does not enumerate.)
...
6. **Peak-bone fire without resonance argument.** Fire on a bone listed in the scene's `peak-bones` array where the interior is fully committed and no displacement-clamp construction earns the load.
7. **Spineless fire (with V3 feel-as-spine equivalence — 2026-05-21).** Memory-flag fire on a beat where narrator-interest is silent AND the V3 feel-as-spine carve-out conditions do not all hold (chapter not `dramatic_shape: hinge`, OR scene's `stakes_axis` not in `axes_held[]`, OR no feel-flag fires on the same `@<proto-line-id>`). The narrator-interest spine OR (held-discipline + feel-flag spine) must register the trigger first. Under the V3 carve-out, feel-flag co-citation is licit; outside it, narrator-interest co-citation is mandatory.
...
- **Sparsity.** 5–12% of bones fire. Sparsity is the load-bearing rubric property.
- **Inverted pressure-signal alignment.** Memory-flags fires concentrate in `flat-low` and `resolving` scene zones (the inverse of narrator-interest, which concentrates on `rising` zones and `peak-bones`). Ratio of fires-per-bone in `flat-low`/`resolving` scenes to fires-per-bone on bones in `peak-bones` arrays should be **at least 3×** (ideally infinite — zero peak-bone fires). The mechanic auditor checks this explicitly by loading the scene-map file (`theater/facets/scene-map-<book>-<chapter>.md`) and classifying each fire's bone against the scene-map's `rhythm-shape` and `peak-bones` fields.
...
- **Quiet-beat distribution.** Fires distribute across the episode's `flat-low` and `resolving` zones, not concentrated in one stretch. A file where all memory-flags fire in @50–@77 (release-zone) is back-loading; a file where all fires cluster at @1–@22 (approach-zone) is front-loading. Both fail file-level: the doubled register should show across the whole episode.

### Scene-level shape

For each scene (per the scene-map file's `@<start>-@<end>` ranges):

- **At most one fire per scene per monument family.** A scene that fires the locker pattern at @33 and again at @34 is over-firing the same monument. Cull to the strongest fire; the second bone is silent.
- **Flat-low and resolving zones permitted-sparse but eligible.** Scenes with `rhythm-shape: flat-low` or `rhythm-shape: resolving` are the primary eligible zones for memory-flag fires — the approach and settle beats can all carry monument-pressure. The default expectation is sparse, not zero.
- **Release zones are the highest-density zones.** Memory and monument fire when the foreground tension releases. Expect `flat-low` and `resolving` scenes to carry the highest density of memory-flag fires per bone — but still constrained by sparsity.
...
- **Pressure-signal inversion fix.** If fires cluster on bones in `peak-bones` arrays, the file is misreading the rubric. Strip the peak-bone fires (none should survive ceiling-defense unless explicit displacement-clamp construction and resonance-not-action argument hold).
...
- **Scene-map (locked, upstream — inverted pressure-signal contract).** Memory-flags fires concentrate in `flat-low` and `resolving` scene zones. Bones listed in `peak-bones` arrays are forbidden by default. The scene-map cross-facet contract is the inverse of narrator-interest's: where narrator-interest concentrates on `rising` zones and `peak-bones`-class beats, memory-flags concentrates in the gaps. Load the scene-map file (`theater/facets/scene-map-<book>-<chapter>.md`) to classify each bone. A bone inside a `peak-bones` array is default-forbidden for memory-flags; the resonance, if it fires, should land in the bones that follow, in the scene's `resolving` or `release-only` tail. Authors should default to firing in the *aftermath* of peak-bones-class beats, not on them.
...
- Scene-map (forward). Memory-flags does not change scene-map fields. The scene-map's `rhythm-shape` and `peak-bones` are upstream-derived from substance_delta; memory-flags reads but does not write them.
...
- **Author:** dialogue-writer fork for the POV character (interiority output mode of the same fork that writes spoken dialogue and authors narrator-interest). For `taylor-hebert-westeros`, the fork loads: base behavior card (§"Memory monuments / Earth-Bet monuments"), Westeros-variant card (§"Memory monuments / Westerosi monuments + local-current"), persona card, the scene-map facet file (`theater/facets/scene-map-<book>-<chapter>.md`) (for inverted pressure-signal check — `rhythm-shape` + `peak-bones` fields replace the pre-overhaul tensometer scalar), the locked narrator-interest file (mandatory spine co-citation), the locked location-state file (soft alignment), and this rubric. **Two-pass authoring:**
  ...
  2. **File-shape pass.** Read the file as a curve. Check chapter-level density (5–12%), inverted pressure-signal alignment (3× or better in `flat-low`/`resolving` zones vs `peak-bones`-class bones), doubled-register visibility (Earth-Bet AND Westerosi clamp both fire), monument-family diversity (≥3 families across the file), quiet-beat distribution (no front- or back-loading). Either fix misfires (NONE→FIRE add for missing coverage; FIRE→NONE strip for density-on-flat) or flag screen-writer kickback for structural gaps. **Do not inflate to hit density.**
- **Reviewer (mechanic auditor):** under this rubric. Per-entry verdict for fires: CORRECT (all three axes earned, no anti-pattern fired) or INCORRECT (named axis-failure or anti-pattern). Per-entry verdict for refusals: CORRECT (no monument earned) or MISSED (a monument-family + cue earned a fire that the author skipped). File-level verdict: SHAPE-OK / SHAPE-FAIL with named density / register / family-diversity / pressure-signal-distribution failure mode. Cross-facet contract pre-ship check is mandatory (narrator-interest co-citation; scene-map inverted pressure-signal alignment; behavior-pack authority).
...
- **Ceiling defense.** If the author defends a FIRE that the reviewer would push to NONE, the burden is on the author to name (a) the monument family, (b) the cue (displacement / clamp / callback), (c) the spine — either the narrator-interest co-citation OR (under V3 carve-out) the feel-flag co-citation with all four carve-out conditions named, (d) the quiet-beat anchor justification, (e) the cross-facet contract slot the entry serves (which metaphor / callback content the licensing fire enables for the stitcher). A FIRE that survives ceiling defense should also pass the doubled-register file-level test and the hard-fence test. When invoking the V3 carve-out, the defense MUST cite the four conditions explicitly: chapter `dramatic_shape`, scene's `stakes_axis` resolution in `axes_held[]`, feel-flag co-citation on the same anchor, and the discipline gates remaining cleared.
- **Cross-author dependencies.** Memory-flags is single-author (POV-character fork). No cross-author dependency check at Phase 5; the cross-facet contract check (vs. narrator-interest, scene-map, behavior pack) replaces it.
