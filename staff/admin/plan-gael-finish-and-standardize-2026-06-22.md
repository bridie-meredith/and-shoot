...
- **Plan B** — harden the local process from the review lessons, then lift it to the repo standard.

The two interleave: Plan B Track 1 (exemplar + hygiene fixes) must land **before** Plan A's `/and-cast`
gate and before chapter production; Plan A's Book-I run is the **live proving ground** for the hardened
process (Plan B Track 2 validation).
...
- **Open decision (principal):** is the and-write `serial` copy now abandoned/downstream, or kept in
  sync? (Drives Plan B Track 2.4.) Recommend: and-shoot becomes canonical for production; and-write
  reconciled later.
...
- **Voice check:** leads (gael, wylla) + the new antagonist/foil exemplars (Plan B) auto-resolve at
  dispatch (Rule 16); `active-project/voice-exemplar.md` calibrates `/and-stitch` Phase 4 voice-embodiment
...
- Discipline: archive-before-mutate, one reason at a time, `/and-write revise` → chain, re-thread,
  re-cohere, rebuild manuscript, stamp parking-lot resolution.
...
filed through the repo's own change-control: an admin **process-proposal**
(`staff/admin/process-proposals.md`, `schemas/admin-proposal.schema.md`) + a **DEC** — so the process
changes are themselves traceable.

1. **Missing marquee exemplars (F1/F6).**
   - Fix: author 4 load-bearing exemplars — `alicent-hightower` (interlude narrator + reclaimed
     warm-cage DNA), `septon-barth`, `jaehaerys-i-targaryen`, `otto-hightower` (voice notes already
     pulled; near-ready), per `schemas/persona-exemplar.schema.md` + `staff/margit/
     exemplar-authoring-process.md`.
   - Guardrail: the gap existed because non-lead Tier-1 exemplars were *optional/queued*, not gated.
     Strengthen `/and-cast` Phase 5 margit gate to block on **every Tier-1 cast member** missing an
     exemplar (POV / interlude-narrator / recurring-comedy roles are impersonator-eligible = Tier-1),
     not just a loose "actor exemplars." Wire into the `and-cast` body + Rule 16.

2. **Stale exemplars mislabeled `dispatch-status: active` (F2/F3).**
   - Fix: retire `saerys-targaryen` / `saerys-septa` / `viserys-i` (`dispatch-status: superseded` +
     `superseded_by`); re-home the septa register to `septa-aldith`; fold viserys warm-cage into Alicent.
   - Guardrail: `dispatch-status` is unmaintained (all 9 say `active`, incl. a superseded one). Add an
     ...
     validation: `superseded_by` ⇒ `dispatch-status: superseded`; an exemplar whose `persona-ref` doesn't
     resolve to a live/library card is flagged (hook into staleness-cascade).
...
3. **Exemplar↔cast roster drift (F1/F4).**
   ...
   - Guardrail: margit coverage report at `/and-cast` + a `/and-review cast` sub-check that diffs roster
     vs. exemplar set (orphans + gaps). Adjacent to the CLAUDE.md "name-novelty / library-slug leak"
     OOS item — same family (live cast not isolated from library/stale artifacts); consider folding.

4. **Rule-22 ledger-register cumulative texture (F5).**
   - Fix: none pre-production (a watch).
...
5. **Meta-lesson: "optional next-step work silently never happens" (root of F1/F6 + the "nothing
   ...
   - Guardrail: route "pending optional elaboration" through the **parking lot** (owning command +
     ...
     readiness gaps** (exemplar coverage, substance-contract completeness) as explicit pre-flight blocks.

## Track B2 — Lift the process to the repo standard (authoring new works)
...
- **B2.1 — Decide the model (open decision, recommend):** *augment*, not *replace*. Keep the proven
  substance pipeline; import the spine-first lessons — the spine as a richer planning artifact feeding
  `/and-substance`; per-chapter narrowing as an `/and-substance chapter` enhancement; the serial
  `/spine`,`/narrow`,`/chapter` ideas as front-end options. (Wholesale front-end replacement is higher
  risk and discards proven machinery.)
- **B2.2 — Generalize project-activation:** a new-work template/checklist embedding the hardened gates —
  exemplar-coverage, substance-contract completeness, Rule-22/readability tracks, consolidated-manuscript
  discipline.
- **B2.3 — Codify in the authoritative surfaces:** `CLAUDE.md` (rules + standardized flow); `RUNBOOK.md`
  (add a "Starting a new work" protocol beside "Producing a chapter" / "Revisions"); command bodies
  (`.claude/commands/*.md`); schemas (exemplar lifecycle, coverage manifest); library assets (margit QC,
  exemplar-authoring-process, audience / orchestrator-critic cards).
- **B2.4 — Reconcile with and-write** (if it stays alive) so the repos don't re-diverge — ties to Plan A
  Phase 0.
- **B2.5 — Validation:** the gael Book-I run (Plan A Phase 2) is the **live proving run** for the
  hardened gates — the same "validate on a live chapter" discipline used for PROP-0032 / the readability
  overhaul.

---

## Cross-plan order of operations
1. **B1 immediate fixes** (4 exemplars + hygiene + the coverage manifest) — required before Plan A
   `/and-cast`.
2. **A Phase 0–1** (re-activate + substance-contract backfill) — with B1 guardrails in place.
3. **A Phase 2, Book I** — the live proving run (B2.5).
4. **Harvest** → refine B1 guardrails + begin B2 codification.
5. **A Books II–III + cohere + revise** (Phases 2–4).
6. **B2 full standardization** + optional and-write reconciliation.
...
- Plan B B2.1: augment vs. replace the substance front-end with spine-first (recommend augment).
- Whether B1 guardrails ship as admin process-proposals for principal triage (the repo's normal path) or
  are applied directly this session.
