...
routing:
  ...
  time_anchor: and-experiment/design/time-anchor.md   # THE single source of truth for STORY time — era (Jaehaerys peace), the anchor formula (Gael age = year_AC − 84, b.84), the per-book/per-chapter AC+age table (Gael/intake 3-book structure, span 84→~95 AC), the drift table, and the ONE open fork (series span: short-escape [default] vs Dance-longevity — needs a principal ruling, §4). Supersedes timeline-and-family-tree.md §1. ⚠ NB: this memory file's `constraints.settings` + `books[].delta` ages below are STALE (Viserys-I era / Saerys / 11-ch Book I) — pre-CL-002 + pre-intake; defer to time_anchor on any date/age/span until the run-03 migration rolls them forward.
  project_history: and-experiment/staff/showrunner/project-history.md   # The PRODUCTION-time chronology — the 8 layers (2026-06-05→08) the project passed through, what each established/superseded, and the drift each left. Read it when files contradict each other. Distinct clock from time_anchor (dev dates vs in-world dates).
