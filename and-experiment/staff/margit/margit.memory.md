...
### Cast-persona library gap — RESOLVED (2026-06-06)

The flag above is cleared. The 5 cast persona warehouse cards were actually `scope: project` with NO library copy (the original-session memory table claiming `both` + library paths was inaccurate on both counts; the files never persisted to `cards/personas/`, on this branch or main). Now corrected: created `cards/personas/{saerys-targaryen,helaena-targaryen-122ac,daenys-velaryon,ser-harwin-the-patient,nymeria-summer-isles}.card.md` (scope:library, project field dropped) and flipped the warehouse copies to `scope: both`. Indexed into cards/personas/INDEX.md (by_world/planetos, by_quality/full; saerys+helaena+daenys added to targaryen-era trope). The cast is now consistently library-resident: persona + behavior + exemplar all present library-side.
