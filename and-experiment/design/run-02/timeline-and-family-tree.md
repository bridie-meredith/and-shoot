# Run-02 — Timeline & Family Tree (AU vs canon)

*Authority for "where/when are we, who is who, and how does magic actually work." Ratified
2026-06-07 (CL-002) from the principal's era + cultivation note. Sits beside
`book-i-state-ledger.md` (the chapter ledger) and `restructured-books-two-desires.md` (the re-axis).
The baseline `convergence/` outline is preserved untouched; this layer supersedes the implicit
Viserys-I-era / Dance-backdrop assumption that the baseline carried.*

**This is an AU (SI-insert).** It diverges from canon deliberately. The job of this file is to make
the divergences explicit and FIXED so every downstream chapter stays consistent. Saerys is
...
a minor young relative "at that time," exactly because his moment (kingship, the Dance) is decades
away.

### Canon date anchors (Fire & Blood — do not contradict)
| AC | Canon event | Use here |
|---|---|---|
| 0 | Aegon's Conquest | deep backdrop |
| 48–103 | **Jaehaerys I reigns** (b. 34 AC) | **our era** |
| ~peak | dragons most numerous in Targaryen history (Dragonpit full) | magic-fuel is *abundant* — matters for cultivation |
| 101 | Great Council names the heir | far-court backdrop (Saerys blind) |
| 103 | Jaehaerys dies (old, mourned) | **candidate father-king gift→spend** (see §5) |
| 103–129 | Viserys I reigns | *after* Book I; Viserys is a child/youth in our window |
| 129–131 | **Dance of the Dragons** — most dragons die | **candidate series climax** (the III.5 "sky of dead ones"), reached via Saerys's longevity — OPEN, see §5 |
...
> **Consistency rule:** any chapter must place Saerys's age = `year_AC − 84`, and must keep the
> political weather matching the year (long peace through 103; only after 103 does the realm tilt
> toward the succession that becomes the Dance). Saerys stays blind to all of it.
...
```
        ...
        (King / father)      (Queen / mother)    [AU: Alicent stands where canon has Alysanne]
                 │
     ┌───────────┼───────────────┐
 [elder brother] │            Helaena            ← Saerys's younger sister; the warm-spot /
 (crown prince)  │            (sister)             riddle-twin, the one who finds her "normal"
     │           │
  Viserys     SAERYS  ◄── protagonist (b. 84 AC), a Targaryen princess; setting-blind;
 (nephew)     dragonless (ate her own cradle-egg, I.1 — reads as a dud)
```

| Person | Role here | Canon-of-record (what we diverged from) |
|---|---|---|
...
| **Otto** | a Hightower at court / the Queen's kin — ambient authority, intelligence source, not antagonist. | Canon: Alicent's father, Hand, Green faction. Demoted to peace-era courtier. |
| **Daenys** | Book II/III warm root. | downstream — re-propagate when those books are touched. |

**Roles transferred by the era move:** the baseline's "warm cage / father" (was Viserys I) →
**Jaehaerys**. The GIFT:VISERYS-father spend → **becomes the Jaehaerys father-king spend** (his
historical 103 AC death is a natural, mourned death she can't fully grieve = armor/curdle). The
young "Viserys" keeps only a minor warm-relation function.
...
## 4. Fence reconciliation (what changed, what held)
...
## 5. The one open structural fork (recommended default, flag for confirm)

**Where does the series end on the canon clock?** The III.5 "sky of dead dragons" rhyme needs a
...
- **ALT — tighter span, AU dragon-death.** Climax near/after Jaehaerys's 103 death with a smaller,
  invented dragon catastrophe; series ends ~110s AC. Keeps a 3-book span tight but loses the
  Dance resonance.

*Pinned for now: Book I dates (§1). The end-date fork is the only thing left genuinely open here.*

---

## 6. Open / defaulted (override anytime)
- **Viserys's exact relation** — nephew (default) vs brother vs cousin.
- **Series end-date fork** — Dance (recommended) vs tighter AU (§5).
- **Alysanne** — written out / replaced by Alicent (default), vs kept as a prior deceased queen.
- **Alicent's arc** — love→horror reversion (default) vs a warm bond the long years spend.
- **Saerys's birth year** — pinned 84 AC; movable ±a few years if a later beat needs it.
